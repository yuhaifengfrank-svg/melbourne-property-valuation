#!/usr/bin/env node
/**
 * Phase 2b: Expert-defined segment OLS — ALL factors included
 *   11 factors + intercept:
 *     logIncome, school, logPop, logUnemp, cbdDist, 
 *     growthPct, highConPct, logSupply,
 *     infraScore, vacancy, popGrowth5yr
 *
 * Plus cash_rate (4.35, constant for all — omitted as it has no variance)
 */
...
async function run() {
  ...
  // === ALL FACTORS ===
  const allVarNames = ['Intercept','logIncome','school','logPop','logUnemp','cbdDist',
    'growthPct','highConPct','logSupply','infraScore','vacancy','popGrowth5yr'];
  // Expected direction: + + + + - - - + - + - +
  //                     intercept, inc, school, pop, unemp, cbd, growth, highCon, supply, infra, vac, popGrowth
  // infra + (more infra = higher price), vac - (higher vac = lower price), popGrowth + (growing = higher price)
  const expectedDirs = ['+','+','+','+','-','-','-','+','-','+','-','+'];

  console.log('================================================================================');
  console.log('=== PER-SEGMENT OLS — ALL 11 FACTORS ===');
  console.log('================================================================================\n');

  let totalR2 = 0, weightedR2 = 0, totalN = 0;
  let totalWeightedAllOK = 0, totalWeightedOKN = 0;
  ...
  segNames.forEach((sn, ci) => {
    ...
    const X = members.map(r => [1, r.logIncome, r.school, r.logPop, r.logUnemp, r.cbdDist,
      r.growthPct, r.highConPct, r.logSupply||0, r.infra||0, r.vac||0, r.popGrowth5yr||0]);
    ...
    console.log(`\n--- ${sn} (n=${members.length}) ---`);
    console.log(`  Profile: $${Math.round(avgPrice/1000)}K CBD=${avgCBD.toFixed(1)}km Sch=${avgSchool.toFixed(0)}`);
    ...
    if(res.n >= res.p+1) {
      ...
      console.log(`  σ(logPrice)=${Math.sqrt(varLogPrice).toFixed(4)}  RMSE=${rmse.toFixed(4)}`);
      ...
      coeffs.forEach(c => {
        ...
        if(ok && c.name !== 'Intercept') okCount++;
      ...
      totalWeightedAllOK += okCount * members.length;
      totalWeightedOKN += (allVarNames.length-1) * members.length;
    }
  });
  ...
  segNames.forEach((sn, ci) => {
    ...
    const segTargets = targets.filter(t => members.some(m => m.sub === t));
    if(segTargets.length === 0) console.log('  (no target suburbs in this segment)');
    
    segTargets.forEach(t => {
      const f = members.find(r => r.sub === t);
      if (!f) return;
      const x = [1, f.logIncome, f.school, f.logPop, f.logUnemp, f.cbdDist,
        f.growthPct, f.highConPct, f.logSupply||0, f.infra||0, f.vac||0, f.popGrowth5yr||0];
  ...
  if(totalWeightedOKN > 0) console.log(`Average direction correctness: ${(totalWeightedAllOK/totalWeightedOKN*100).toFixed(1)}%`);
