...
if (!files.length) throw new Error("No City of Melbourne development metric artifacts found");
...
if (sourceRows.size !== 1 || accountedRows !== [...sourceRows][0]) {
  throw new Error("City of Melbourne aggregate reconciliation failed");
}
...
const verified = await sql`
  ...
  WHERE LOWER(council) = LOWER(${"City of Melbourne"})
    AND snapshot_at = ${metrics[0].snapshotAt}
`;
if (verified[0].count !== metrics.length || verified[0].accounted !== accountedRows) {
  throw new Error("Post-write City of Melbourne verification failed");
}
console.log(JSON.stringify({
  mode: "write-database",
  council: "City of Melbourne",
