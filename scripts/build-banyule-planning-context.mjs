...
const ANNUAL_REPORT = "https://www.banyule.vic.gov.au/files/assets/public/v/1/about-us/strategies/tbs-24-25-the-banyule-story-2024%E2%80%932025.pdf";
...
const NOTICE_REGISTER = "https://www.banyule.vic.gov.au/Planning-building/Review-local-planning-applications/Planning-applications-on-public-notice";
const AREAS = [
  ["BELLFIELD", "3081"], ["BRIAR HILL", "3088"], ["BUNDOORA", "3083"],
  ["EAGLEMONT", "3084"], ["ELTHAM", "3095"], ["ELTHAM NORTH", "3095"],
  ["GREENSBOROUGH", "3088"], ["HEIDELBERG", "3084"], ["HEIDELBERG HEIGHTS", "3081"],
  ["HEIDELBERG WEST", "3081"], ["IVANHOE", "3079"], ["IVANHOE EAST", "3079"],
  ["LOWER PLENTY", "3093"], ["MACLEOD", "3085"], ["MONTMORENCY", "3094"],
  ["ROSANNA", "3084"], ["SAINT HELENA", "3088"], ["VIEWBANK", "3084"],
  ["WATSONIA", "3087"], ["WATSONIA NORTH", "3087"], ["YALLAMBIE", "3085"],
].map(([suburb, postcode]) => ({ suburb, postcode, activityCentre: suburb === "HEIDELBERG" }));
...
function artifact(area) {
  return {
    ...
    sources: [
      {
        publisher: "Banyule City Council",
        url: ANNUAL_REPORT,
        report: "The Banyule Story 2024-2025",
    ...
    councilPlanningService: {
      medianDecisionDays: 17,
      medianDecisionDaysTarget: 25,
      decidedWithinRequiredTimePercent: 87.64,
      decidedWithinRequiredTimeTargetPercent: 78,
    ...
    activityCentre: area.activityCentre ? {
      included: true,
      programGroup: "stage_1",
      description: "Heidelberg is included in Stage 1 of Victoria's Train and Tram Zone Activity Centres Program",
    } : {
      included: false,
    ...
    publication: {
      ...
      limitations: [
        ...
        `The public-notice register at ${NOTICE_REGISTER} is not collected or republished because its published terms prohibit using applicant information to identify and market products or services`,
...
<section class="ahv-planning" aria-labelledby="ahv-banyule-planning-heading">
  ...
  <div class="ahv-planning-grid">
    <article class="ahv-planning-card"><div class="ahv-planning-label">Median decision time</div><div class="ahv-planning-value">17 days</div><div class="ahv-planning-meta">Council target: 25 days</div></article>
    <article class="ahv-planning-card"><div class="ahv-planning-label">Within required time</div><div class="ahv-planning-value">87.64%</div><div class="ahv-planning-meta">Council target: 78%</div></article>
${activity}
  </div>
</section>
<!-- AHV_BANYULE_PLANNING_CONTEXT_END -->
`;
}

function pageTemplate(area) {
  const name = title(area.suburb);
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${name} VIC Planning Research | AusHomeValue</title><meta name="description" content="Source-labelled Banyule planning context for ${name}, Victoria."><link rel="canonical" href="https://www.aushomevalue.com.au/suburb/${slug(area.suburb)}-vic.html"><meta name="robots" content="index, follow"><link rel="stylesheet" href="/shared-responsive.css"><style>*{box-sizing:border-box}body{margin:0;background:#f4f6f5;color:#17211d;font-family:Inter,system-ui,-apple-system,sans-serif;line-height:1.55}.topbar{background:#0d6b57;padding:14px 24px}.topbar a{color:#fff;text-decoration:none;font-weight:700}.container{max-width:960px;margin:auto;padding:32px 20px 12px}.breadcrumb,.eyebrow,.notice{color:#66736d}.notice{max-width:920px;margin:0 auto 48px;padding:14px 16px;background:#e8f3ef;border-left:4px solid #0d6b57;border-radius:6px}</style></head><body><div class="topbar"><a href="/">← AusHomeValue</a></div><main class="container"><div class="breadcrumb"><a href="/">Home</a> / <a href="/suburb-research.html">Suburb Research</a> / ${name}</div><h1>${name}, VIC — Planning Research</h1><p class="eyebrow">Postcode ${area.postcode} · Banyule council context · Only verified, source-labelled metrics are shown.</p></main><!-- AHV_BANYULE_PLANNING_CONTEXT_INSERT --><div class="notice"><strong>Data availability:</strong> suburb-level application counts are omitted because no approved reusable source is available.</div></body></html>`;
}

for (const area of AREAS) {
  const data = artifact(area);
  fs.writeFileSync(path.join(ROOT, "data", "validation", `${slug(area.suburb)}-banyule-planning-context-2025.json`), `${JSON.stringify(data, null, 2)}\n`);
  ...
  html = html.replace(/[\r\n]*<!-- AHV_BANYULE_PLANNING_CONTEXT_START -->[\s\S]*?<!-- AHV_BANYULE_PLANNING_CONTEXT_END -->[\r\n]*/g, "");
  html = html.includes("<!-- AHV_BANYULE_PLANNING_CONTEXT_INSERT -->")
    ? html.replace("<!-- AHV_BANYULE_PLANNING_CONTEXT_INSERT -->", markup(data))
    : html.replace("</body>", `${markup(data)}</body>`);
  fs.writeFileSync(page, html);
}

console.log(JSON.stringify({ areas: AREAS.length, councilMetrics: { medianDecisionDays: 17, withinRequiredTimePercent: 87.64 } }));
