...
### 7.7 Example back-test workflow

```text
Target:
9 Macedon Avenue, Balwyn North
Actual sale price hidden

Data allowed:
Only sales before 26 Jul 2025

Model:
Build comparable baseline from earlier Balwyn North / BHSZ / 600-800 sqm house sales
Apply land, building, condition, micro-location and planning/risk adjustments

Output:
Estimated range and midpoint

Validation:
Reveal actual sale price: $2.975m
Compare midpoint and range hit
Record error and cause
```

### 7.8 Balwyn North v0 manual back-test

第一批人工回测用于验证模型方向。该批次不是严格盲测, 因为当前还没有历史成交数据库自动隐藏成交价; 但可作为模型校准参考。
...
- Balwyn High School Zone is a strong positive factor, but not sufficient by itself.
- Street rank, dwelling condition, floorplan, and buyer demand materially affect the final price.
- Older large-land houses should not automatically be valued like renovated family homes.
- Strongly renovated / contemporary family homes need a higher condition and building utility adjustment.
- If model overestimates BHSZ houses with weaker condition or less compelling street rank, increase condition sensitivity and widen uncertainty.

Impact on 34 Macedon Avenue sample:

```text
The Balwyn North v0 back-test supports the earlier 34 Macedon Avenue estimate:
...
- BHSZ and village walkability are positive
...
## 8. Sample Property: 34 Macedon Avenue, Balwyn North

该地址作为第一个样本, 用于验证 House MVP 字段和 micro-location 模型。
...
- Bedrooms: 5, or 4 bedrooms plus study depending listing description
- Bathrooms: 2
- Car spaces: 2
...
### 8.2 初步 micro-location 判断

- Suburb: Balwyn North, mature family-oriented suburb
- Street type: likely residential street, not a main road
- School zone premium: strong
- Amenity walkability: strong
- Negative externalities: needs further check
- Overall micro-location: likely positive within Balwyn North, subject to Street View, planning overlays, and recent comparable sales confirmation

### 8.3 当前缺失数据

要把 34 Macedon Avenue 的估值置信度从 Medium 提升到 Medium-High / High, 需要补齐:

- Current condition evidence: 当前照片、维护状态、是否翻新、结构或布局问题
- Title and planning check: title type、zoning、overlay、heritage、easement、covenant、flood / bushfire risk
- Complete recent comparable sales set: 最近 6-12 个月完整同区、同学区、同土地/房型/微位置的成交池
