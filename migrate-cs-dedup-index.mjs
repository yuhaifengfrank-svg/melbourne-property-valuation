...
// 使用 sql.transaction(...) 保证原子性（Neon HTTP 需要同一会话）
...
async function main() {
  ...
  // Neon @neondatabase/serverless + @vercel/postgres 支持 sql.transaction
  await sql.transaction([
    // 步骤 1：清理重复数据 — 同一 (address, COALESCE(date,'1970-01-01'), COALESCE(price,-1), source)
    ...
    // 步骤 2：删除旧索引
    `DROP INDEX IF EXISTS cs_dedup_idx`,
    // 步骤 3：创建 COALESCE 版
    ...
    queryMode: "transaction"  // Neon 保证同一会话
  });
