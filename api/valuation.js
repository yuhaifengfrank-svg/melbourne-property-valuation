...
export default async function handler(request, response) {
  ...
  if (request.method === 'GET' && (request.query?.uv || request.query?.opportunities)) {
    const { default: uvHandler } = await import('../lib/uv-api-handler.js');
    return uvHandler(request, response);
  }
  ...
  try {
    ...
    if (body.debug) {
      freeSummary._debug = { 
        ...
        acceptedComparables: (result.valuation?.acceptedComparables || []).slice(0, 20).map(c => ({
          price: c.salePrice,
          suburb: c.address?.split(',')[1]?.trim() || '?',
          street: c.address?.split(',')[0] || '?',
          bedrooms: c.bedrooms,
          bathrooms: c.bathrooms,
          landSize: c.landSize,
          distanceMeters: c.distanceMeters,
