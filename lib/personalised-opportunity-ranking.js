...
export function calculatePersonalisedScore(baseScore, suburb, opts) {
  ...
  const comparableCount = Number(suburb.overallConfidence) || 0;
  const vacancyRate = Number(suburb.vacancyRate) || 0;
  const supplyRatio = Number(suburb.supplyConstraintScore) || 0;
  ...
  switch (goal) {
    case "growth": {
      ...
      const supplyBonus = supplyConstraintScore !== null && supplyConstraintScore < 30 ? 1.5 : 0;
  ...
  switch (goal) {
    case "growth":
      return `Strong underlying demand signals${
        ...
        supplyConstraintScore !== null && supplyConstraintScore > 0 && supplyConstraintScore < 30
...
export function generateRisk(suburb) {
  ...
  if (supplyConstraintScore !== null && supplyConstraintScore > 60) {
