...
//
// This module is called AFTER /api/opportunity returns its raw Future
// Opportunity Index results. This module re-ranks by user's goal.

/**
 * Rank suburbs by personalised criteria.
 * @param {Array} opportunities - Raw suburbs from /api/opportunity?strategy=<goal>
...
export function rankPersonalised(opportunities, preferences) {
  ...
  const maxScore = Math.max(...filtered.map((s) => baseFutureScore(s)), 1);

  const scored = filtered.map((suburb) => {
    const baseScore = baseFutureScore(suburb);
    ...
    return {
      ...
      baseFutureScore: Math.round(baseScore * 100) / 100,
      personalisedFutureScore: Math.round(result.personalisedScore * 100) / 100,
      futureOpportunityIndex: Math.round(baseScore * 100) / 100,
      band: suburb.band || "",
      forecastHorizon: suburb.forecastHorizon || "3-5 years",
      ...
      disclaimer:
        "Future Opportunity Index is a relative 0-100 screening signal. It is not a price forecast, financial advice, or a guaranteed return.",
    };
  });
...
function baseFutureScore(suburb) {
  const canonicalValue = suburb?.score?.value;
  const canonical = canonicalValue === null || canonicalValue === undefined || canonicalValue === ""
    ? NaN
    : Number(canonicalValue);
  if (Number.isFinite(canonical)) return canonical;
  const future = Number(suburb.futureOpportunityIndex);
  if (Number.isFinite(future)) return future;
  const futureAlt = Number(suburb.suburbFutureScore);
  if (Number.isFinite(futureAlt)) return futureAlt;
  return Number(suburb.opportunityScore) || 0;
}
...
export function calculatePersonalisedScore(baseScore, suburb, opts) {
  ...
  const overallConfidence = Number(suburb.overallConfidence) || 0;
  const vacancyRate = Number(suburb.vacancyRate) || 0;
  const supplyConstraintScore = Number(suburb.supplyConstraintScore) || 0;
  ...
  switch (goal) {
    case "growth": {
      // Use demand signals: low vacancy + supply constraint + data coverage
      const demandBonus =
        vacancyRate < 3 ? 2 : vacancyRate < 5 ? 1 : 0;
      // FIX: High supply constraint score = tight supply = positive for growth
      const supplyBonus = supplyConstraintScore !== null && supplyConstraintScore > 60 ? 1.5 : 0;
      const dataBonus = overallConfidence >= 70 ? 0.5 : 0;
  ...
  // Confidence adjustment — based on overall_confidence (0-100, higher = better data)
  // overallConfidence is NULL when not computed; treat as low
  if (suburb.overallConfidence === null || suburb.overallConfidence === 0) {
    // No confidence data at all — moderate penalty
    adjustment -= 1;
  } else if (suburb.overallConfidence < 30) {
    adjustment -= 2;
  } else if (suburb.overallConfidence < 50) {
    adjustment -= 1;
  }
  ...
  // FIX 7: Clamp final score to 0-100
  const personalisedScore = clamp(baseScore + adjustment, 0, 100);
  adjustment = personalisedScore - baseScore;
...
export function generateReason(suburb, goal) {
  if (Array.isArray(suburb.why) && suburb.why.length > 0) {
    return suburb.why.slice(0, 3).join("; ");
  }
  const rentalYield = Number(suburb.rentalYield) || 0;
  const schoolScore = Number(suburb.schoolScore) || 0;
  const vacancyRate = Number(suburb.vacancyRate) || 0;
  ...
  switch (goal) {
    case "growth":
      return `Strong underlying demand signals${
        ...
        // FIX: High = tight supply = favourable supply dynamics
        supplyConstraintScore !== null && supplyConstraintScore > 60
          ? ", favourable supply dynamics"
          : supplyConstraintScore !== null && supplyConstraintScore < 30
          ? ", elevated new supply pipeline"
          : ""
      }${
        overallConfidence !== null && overallConfidence >= 60 ? " with strong data coverage" : ""
      }`;
    case "cashflow":
...
export function generateRisk(suburb) {
  if (Array.isArray(suburb.risks) && suburb.risks.length > 0) {
    return suburb.risks.slice(0, 3).join(" · ");
  }
  const risks = [];
  const rentalYield = Number(suburb.rentalYield) || 0;
  const vacancyRate = Number(suburb.vacancyRate) || 0;
  ...
  // FIX: Low supply constraint score = more new supply = risk; high score = tight supply = no risk
  if (supplyConstraintScore !== null && supplyConstraintScore < 30) {
    risks.push("Elevated new supply may outpace demand");
  }
  if (overallConfidence > 0 && overallConfidence < 30) {
    risks.push("Limited market data coverage — approach with caution");
  }
  if (risks.length === 0) {
...
export function dataSourceConfidence(suburb) {
  if (typeof suburb.confidence === "string" && suburb.confidence) return suburb.confidence;
  const futureConfidence = Number(suburb.confidenceScore);
  if (Number.isFinite(futureConfidence)) {
    if (futureConfidence >= 75) return "High";
    if (futureConfidence >= 50) return "Medium";
    if (futureConfidence >= 30) return "Low";
    return "Very Low";
  }
  const count = Number(suburb.overallConfidence) || 0;
