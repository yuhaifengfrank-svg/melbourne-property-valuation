...
export async function listWatchItems(sql, leadContactId, includeArchived = false) {
  return sql`
    SELECT id, item_type, canonical_item_key, suburb, state, postcode,
           ...
           latest.future_opportunity_score,
           latest.confidence_score,
           latest.model_version AS score_model_version,
           latest.data_as_of AS score_data_as_of,
           previous.future_opportunity_score AS previous_opportunity_score
    FROM investor_watch_items i
    LEFT JOIN LATERAL (
      SELECT future_opportunity_score, confidence_score, model_version, data_as_of
      FROM investor_watch_score_history
      WHERE watch_item_id = i.id
      ORDER BY captured_at DESC, id DESC LIMIT 1
    ) latest ON TRUE
    LEFT JOIN LATERAL (
      SELECT future_opportunity_score
      FROM investor_watch_score_history
      WHERE watch_item_id = i.id
      ORDER BY captured_at DESC, id DESC OFFSET 1 LIMIT 1
    ) previous ON TRUE
    WHERE i.lead_contact_id = ${leadContactId}
      AND (${includeArchived}::boolean OR i.status = 'active')
    ORDER BY i.updated_at DESC, i.id DESC
  `;
}

export async function listWatchHistory(sql, leadContactId, itemId, limit = 30) {
  const id = positiveId(itemId);
  const safeLimit = Math.min(Math.max(Number(limit) || 30, 1), 100);
  return sql`
    SELECT h.future_opportunity_score, h.confidence_score, h.component_scores,
           h.model_version, h.data_as_of, h.captured_at
    FROM investor_watch_score_history h
    JOIN investor_watch_items i ON i.id = h.watch_item_id
    WHERE h.watch_item_id = ${id} AND i.lead_contact_id = ${leadContactId}
