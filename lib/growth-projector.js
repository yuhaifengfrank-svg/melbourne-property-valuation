...
async function computeBaseRate(suburb) {
  ...
  if (rows.length < 2) {
    // D) VGV official CAGR (10-year rolling average from Valuer-General Victoria)
    // More reliable than pooled regression when suburb has few sales
...
async function fallbackVGV(suburb) {
  const db = sql();
  const row = await db.query('SELECT govt_5yr_cagr FROM suburb_metrics WHERE suburb = $1 AND govt_5yr_cagr IS NOT NULL', [suburb]);
  ...
  for (const row of suburbs) {
    try {
      ...
      await db.query(
        ...
        [g1y, Math.round(result.growth3y * 100) / 100, Math.round(result.growth5y * 100) / 100, row.suburb]
      );
      done++;
