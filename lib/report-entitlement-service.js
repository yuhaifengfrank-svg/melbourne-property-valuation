 ...
 */
// ── ContactId-based entry point (Phase 1E3C-3A) ─────────────────────

/**
 * Check if a user (identified by leadContactId from signed cookie) is entitled
 * to access a paid report.
 *
 * Shares resolvePaymentForContact / parseSnapshotJson with the email version.
 * Does NOT fetch lead_contacts (caller provides contactId from cookie).
 * Does NOT accept email.
 *
 * @param {object} params
 * @param {string} params.reportId  — The report_id to check
 * @param {number} params.leadContactId  — Lead contact id from verified cookie
 * @param {import("@neondatabase/serverless").NeonQueryFunction} sql  — Neon SQL tagged template
 * @returns {Promise<object>} Same shape as checkReportEntitlement
 * @throws {EntitlementCheckError} with .code from REJECTION constants
 */
export async function checkReportEntitlementByContactId({ reportId, leadContactId }, sql) {
  // ── 1. Validate input ──────────────────────────────────────────
  ...
  if (leadContactId === undefined || leadContactId === null ||
      typeof leadContactId !== "number" || !Number.isInteger(leadContactId) ||
      leadContactId <= 0) {
    throw new EntitlementCheckError(
      REJECTION.NOT_FOUND,
  ...
  const contactId = leadContactId;

  // ── 2. Query each report table separately ─────────────────────
  ...
  // ── 3. Determine which records exist ───────────────────────────
  ...
  // ── 5. Resolve payment for this contact ────────────────────────
