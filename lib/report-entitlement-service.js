...
// ── Resolve payment for this contact across possibly multiple rows ──

/**
 * Fetch all payments for a report_id and find the one belonging
 * to the requesting contact. If none belonging to this contact exist
 * but payments from others do → REPORT_OWNER_CONFLICT.
 * If no payments at all → PAYMENT_NOT_CONFIRMED.
 *
 * Returns the matched payment row, or throws EntitlementCheckError.
 */
async function resolvePaymentForContact(reportId, contactId, sql) {
  ...
  // Deterministic: use the most recent (highest id) record for this contact
  // If there are multiple (anomaly), select the latest
  if (myPayments.length > 1) {
    myPayments.sort((a, b) => Number(b.id) - Number(a.id));
  }
...
export async function checkReportEntitlement({ reportId, email }, sql) {
  ...
  //
  // report_payments deliberately fetches ALL rows for this report_id so
  // we can filter by contact owner and handle multi-row scenarios.
  ...
  // ── 6. Strict owner checks ─────────────────────────────────────
  //
  // All three records must belong to the requesting contact.
  ...
  // ── 7. Resolve payment for this contact ────────────────────────
  //
  // Multiple payments can exist for the same report_id (e.g. retried
  // checkouts).  We must find the one belonging to the requesting
  // contact.  If only other-customer payments exist → conflict.
  // If no payments at all (already caught above).

  const pay = await resolvePaymentForContact(reportId, contactId, sql);
