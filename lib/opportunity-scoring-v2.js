...
export async function scoreAllSuburbs() {
  ...
  for (const [suburb, m] of metrics) {
    const strat = classifySuburb(m);
    const score = computeSuburbScore(m, strat);
    try {
      await q(
...
function classifySuburb(m) {
  ...
  const g = Number(m.growth_1y) || 0;
  const g3 = Number(m.growth_3y) || 0;
  const y = Number(m.gross_yield) || 0;
  const v = Number(m.vacancy_rate) || 10;
  const s = Number(m.school_score) || 0;
  const infra = Number(m.infrastructure_score) || 0;

  const gScore = Math.min(100, Math.max(0, (g + g3 * 2) / 3));
  const isVacation = (m.vacancy_rate != null && Number(m.vacancy_rate) < 1.0);
...
function computeSuburbScore(m, classification) {
  ...
  const g = Number(m.growth_1y) || 0;
  const g3 = Number(m.growth_3y) || 0;
  const g5 = Number(m.govt_5yr_cagr) || 0;
  const y = Number(m.gross_yield) || 0;
  const v = Number(m.vacancy_rate) || 10;
  const s = Number(m.school_score) || 0;

  // Normalize each factor to 0-100
  const undervalScore = Math.min(100, u * 2);
  const growthScore = Math.min(100, Math.max(0, (g + g3 * 2 + g5 * 2) / 3));
  const yieldScore = Math.min(100, y * 25);
  const vacancyScore = Math.min(100, Math.max(0, (10 - v) * 10));
  const schoolScore = Math.min(100, s);
  const confidenceScore = (u > 0 && g3 > 0 ? 40 : 20) + (g5 > 0 ? 30 : 10) + (s > 0 ? 30 : 10);
  ...
  return { opportunityScore: Math.min(99, finalScore), classification };
}
