...
function calcRentalScoresV2(m) {
  ...
  if (m.gross_yield != null) {
    ...
    yldScore = Math.round(Math.max(0, Math.min(100, m.gross_yield * 20)));
  } else {
  ...
  if (m.vacancy_rate_adjusted != null) {
    // 0% = 100, 5% = 0
    vacScore = Math.round(Math.max(0, Math.min(100, 100 - m.vacancy_rate_adjusted * 10)));
  } else {
...
// ── Batch score all suburbs (for nightly refresh) ──
export async function scoreAllSuburbs() {
  const metrics = await loadAllMetrics();
  if (!metrics || metrics.size === 0) {
    console.log('[v2-scoring] No metrics to score');
    return { scored: 0 };
  }

  let scored = 0;
  for (const [suburb, m] of metrics) {
    const score = computeSuburbScore(m, {});
    const strat = classifyOpportunityV2(
      ...
      score.vacancyScore,
      score.schoolScore,
      m.infrastructure_score || 0
    );
    try {
      await q(
        'UPDATE suburb_metrics SET opportunity_score = $1::numeric(5,1), opportunity_type = $2::text, updated_at = NOW() WHERE LOWER(suburb) = LOWER($3)',
        [score.opportunityScore, strat.label, suburb]
      );
      scored++;
    } catch (e) {
      console.warn('[v2-scoring] Fail:', suburb, e.message.substring(0, 100));
    }
  }
  console.log('[v2-scoring] Scored', scored, 'suburbs');
  return { scored };
}

// ── Classify a suburb into an opportunity type ──
function classifySuburb(m) {
  const u = Number(m.undervaluation) || 0;
  const gs = Number(m.growth_score) || 0;
  const y = Number(m.gross_yield) || 0;
  const v = Number(m.vacancy_rate_adjusted) || 10;
  const s = Number(m.school_score) || 0;
  const infra = Number(m.infrastructure_score) || 0;

  const gScore = Math.min(100, Math.max(0, gs));
  const isVacation = (m.vacancy_rate_adjusted != null && Number(m.vacancy_rate_adjusted) < 1.0);

  // Threshold-driven classification
  let label = 'Balanced Opportunity';

  if (u >= 15 && gScore >= 8 && s >= 60 && y >= 2.0) label = 'Smart Buy';
  else if (gScore >= 15) label = 'Growth Opportunity';
  else if (y >= 3.0 && v < 5) label = 'Cashflow Opportunity';
  else if (s >= 65) label = 'School Zone Opportunity';
  else if (u >= 20) label = 'Value Opportunity';
  else if (infra >= 30) label = 'Infrastructure Opportunity';

  if (isVacation && (label.includes('Cashflow') || label.includes('Value'))) {
...
// ── Compute suburb-level score ──
function computeSuburbScore(m, classification) {
  const u = Number(m.undervaluation) || 0;
  const gs = Number(m.growth_score) || 0;
  const y = Number(m.gross_yield) || 0;
  const v = Number(m.vacancy_rate_adjusted) || 10;
  ...
  // undervaluation is a Z-score (~-1.6 to ~+1.2), map to 0-100
  const undervalScore = Math.min(100, Math.max(0, Math.round((u + 1.6) / 2.8 * 100)));
  const growthScore = Math.min(100, Math.max(0, gs));
  const yieldScore = Math.min(100, y * 20);
  const vacancyScore = Math.min(100, Math.max(0, 100 - v * 10));
  const schoolScore = Math.min(100, s);
  // Confidence = data completeness for all 5 main dimensions
  // Each dimension: null check (Z-score ≠ 0 means data exists, even if negative)
  const hasU = u != null && u !== 0;
  const hasG = gs != null;
  const hasY = y != null && y > 0;
  const hasV = v != null && v > 0;
  const hasS = s != null && s > 0;
  const fieldsPresent = [hasU, hasG, hasY, hasV, hasS].filter(Boolean).length;
  const confidenceScore = Math.round(fieldsPresent / 5 * 100);
  ...
  return { opportunityScore: Math.min(99, finalScore), undervalScore, growthScore, yieldScore, vacancyScore, schoolScore, classification };
}
...
export async function scanOpportunitiesV2(params = {}) {
  ...
  // Deduplicate: only highest-scoring property per suburb (case-insensitive)
  const seenSuburbs = new Map();
  const deduped = [];
  for (const s of filtered.sort((a, b) => b.opportunityScore - a.opportunityScore)) {
    var key = s.suburb ? s.suburb.toLowerCase() : "";
    if (!seenSuburbs.has(key)) {
      seenSuburbs.set(key, s.suburb);
      deduped.push(s);
    }
  }
...
function scorePropertyV2(sale, metrics, strategy) {
  ...
  let n = 0;
  if (med != null) n++;
  if (metrics.growth_score != null) n++;
  ...
  return {
    ...
    riskFactors: riskFactorsV2(uvPct, metrics.growth_3y, metrics.vacancy_rate_adjusted),
  };
}
...
function calcGrowthScoreV2(m) {
  // Use pre-computed suburb_metrics.growth_score
  const gs = m.growth_score;
  if (gs != null && !isNaN(gs)) {
    return Math.max(0, Math.min(100, gs));
  }
...
function classifyOpportunityV2(score, uv, gt, yld, vac, sch, infra) {
  ...
  const catScores = [
    { t: 'school', s: Math.min(100, sch),
      adjust: sch >= 75 ? 0 : -20, },  // only truly good schools
    { t: 'growth', s: Math.min(100, gt),
      adjust: gt >= 60 ? 0 : -10, },
    { t: 'cashflow', s: Math.round(0.6 * yld + 0.4 * vac),
      adjust: yld >= 60 ? 0 : -10, },
