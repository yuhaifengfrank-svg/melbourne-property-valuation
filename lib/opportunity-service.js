...
// Module-level pg pool — TCP connection, faster cold-start than neon HTTP transport
let _pool = null;
function getPool() {
  if (!_pool && process.env.DATABASE_URL) {
    _pool = new pgPool(new pg.Pool({ connectionString: process.env.DATABASE_URL }));
  }
  if (!_pool) throw new Error('DATABASE_URL not set');
  return _pool;
}
...
async function loadSuburbData(suburbs, sql) {
  ...
  if (codes.length > 0) {
    ...
    const incomeMap = {
      ...
      median_family_income_weekly: 'Median_fam_inc_weekly',
...
export async function scanOpportunities(params = {}) {
  ...
  let suburbData = {};
  try {
    const batchPromise = loadSuburbData(allSuburbs, sql);
    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('batch_data_timeout')), 4000));
...
function calcCashFlowScore(sd) {
  const cens = sd.census;
  if (!cens || !cens.median_rent_weekly) return 40;
  const rw = cens.median_rent_weekly || 0;
  if (rw <= 0) return 40;
