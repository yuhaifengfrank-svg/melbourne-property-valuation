...
export async function getPlanningSignals(sql, lat, lng) {
  ...
  try {
    ...
    // Only write back if coordinates are within the Melbourne metro bounding box.
    // This prevents Nominatim errors (e.g. Box Hill NSW → FZ1 written to cache)
    // from polluting the cache with wrong zone data.
    const MELB_LAT_MIN = -38.7;
    const MELB_LAT_MAX = -37.2;
    const MELB_LON_MIN = 143.8;
    const MELB_LON_MAX = 145.8;
    const isWithinMelbourne = (
      lat >= MELB_LAT_MIN && lat <= MELB_LAT_MAX &&
      lng >= MELB_LON_MIN && lng <= MELB_LON_MAX
    );

    if (zones.length > 0 && isWithinMelbourne) {
      try {
        const z = zones[0];
