...
function escGeom(geom, simplifyTolerance = 0) {
  const raw = `ST_SetSRID(ST_GeomFromGeoJSON('${JSON.stringify(geom).replace(/'/g, "''")}'), 4326)`;
  const tolerance = Number(simplifyTolerance);
  if (Number.isFinite(tolerance) && tolerance > 0) {
    return `ST_SimplifyPreserveTopology(${raw}, ${tolerance})`;
  }
  return raw;
}
...
function toValueRow(f, options = {}) {
  ...
  const cols = [
    ...
    escNum(p.ufi),
    escGeom(f.geometry, options.simplifyTolerance),
    "NOW()",
  ];
  return `(${cols.join(", ")})`;
}

async function batchedUpsert(sql, table, features, options = {}) {
  let total = 0;
  ...
  for (let i = 0; i < features.length; i += DB_BATCH) {
    ...
    const valueRows = batch.map((feature) => toValueRow(feature, options));
