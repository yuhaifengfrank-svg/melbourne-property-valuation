...
export const TARGET_DATA_CUTOFF = "2025-12-31";
...
export function estimateRentalVacancy({ uniqueListings21Days, estimatedRentalStock,
  ...
  if (vacancies == null || vacancies < 0 || stock == null || stock <= 0) return buildMetric({
    ...
    limitations: ["Valid local vacancy listings and rental-stock estimate are required."],
  });
  ...
  return buildMetric({
    ...
    confidence: stock >= 500 && vacancies >= 10 ? "medium" : "low",
    range: { low: round(clamp(rate - error, 0, 1) * 100), high: round(clamp(rate + error, 0, 1) * 100) },
    limitations: ["Modelled from deduplicated listings and estimated rental stock; not the 2021 Census unoccupied-dwelling rate."],
  });
}
