...
export function addressSignature(address, propertyType = "") {
  const normalized = normalizeAddress(address);
  const attachedType = /^(unit|townhouse|villa|apartment)$/i.test(String(propertyType || ""));
  const hyphenMatch = normalized.match(/^([a-z\d]+)\s*-\s*(\d+[a-z]?)\s+/i);
  const hyphenIsUnit = Boolean(attachedType && hyphenMatch);
  const parsed = hyphenIsUnit
    ? normalized.replace(/^([a-z\d]+)\s*-\s*(\d+[a-z]?)\b/i, "$1/$2")
    : normalized;
  const slashMatch = parsed.match(/\b([a-z\d]+)\s*\/\s*(\d+[a-z]?)\b/i);
  const unitMatch = parsed.match(/\bunit\s+([a-z\d]+)\b/i);
  const apartmentMatch = parsed.match(/\b(?:apartment|apt|flat)\s+([a-z]?\d+[a-z]?)\b/i);
  ...
  return {
    ...
    hasUnitSignal: hyphenIsUnit || /\bunit\b|\bapt\b|\bapartment\b|\bflat\b|^\s*[a-zA-Z\d]+\s*\//i.test(String(address || "")),
    ambiguousHyphen: Boolean(hyphenMatch && !attachedType),
  };
}
...
export async function buildSubject(input) {
  ...
  const signature = addressSignature(enteredAddress, propertyType);
  ...
  try {
    ...
    let queryAddress = enteredAddress
      ...
      .replace(/^\d+\s*\//, "")
      .trim();
    if (/^(unit|townhouse|villa|apartment)$/i.test(propertyType)) {
      queryAddress = queryAddress.replace(/^[a-z\d]+\s*-\s*(\d+[a-z]?)\b/i, "$1").trim();
    }
        ...
        if (mismatchFields.length === 0) {
          ...
          const unitPrefix = hasUnitInput
            ? (enteredAddress.match(/^(?:unit\s+)?\d+(?=\s*\/)/i)?.[0]
               || enteredAddress.match(/^(?:unit\s*)?(\d+)/i)?.[1]
               || "")
            : "";
