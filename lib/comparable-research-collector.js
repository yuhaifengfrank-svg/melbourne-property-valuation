...
export async function buildSubject(input) {
  ...
  //  地址核验逻辑
  //  规则：
  //  - 双方都有值且明确不同 → mismatch（只对明确的冲突才拒绝）
  //  - Map 缺字段 (unconfirmed) → 用客户输入，降低置信度，不报 mismatch
  //  - Unit 号若地图无法核验 → unitStatus: unverified，可继续估值
  //  - suburb/state 精确比较，full-name→abbreviation 映射
  //  - houseNum + road 同时有值才比较，缺一不报 mismatch
  //  - 最终 subject.address = canonicalAddress
  // ═══════════════════════════════════════════════════════════════
  try {
    // 查询 Nominatim 时去掉 unit/apt 前缀，否则 "Unit 3/18 Moresby St" 格式无法解析
    const queryAddress = enteredAddress.replace(/^(unit|apt|apartment)\s+\d+\s*\/\s*/i, "").trim();
    const v = await verifyAddress(queryAddress + " " + state);
    if (v.ok) {
      const vSuburb = v.suburb.replace(/,.*$/, "").trim();
      const vRoad = clean(v.road);
      const vHouseNum = clean(v.streetNumber);
      ...
      // ── Suburb 精确相等 ──
      const suburbExact = Boolean(vSuburb) &&
        titleCase(vSuburb).toLowerCase() === suburb.toLowerCase();
      verifier.suburbExact = suburbExact;
      const suburbConfirmed = Boolean(vSuburb);
      ...
      const stateConfirmed = Boolean(verifier.verifiedState);

      // ── 门牌号匹配：仅双方都有值时才比较；缺失时算 unconfirmed ──
      const houseNumConfirmed = Boolean(vHouseNum && inputHouseNum);
      const houseNumMatch = houseNumConfirmed && vHouseNum === inputHouseNum;
      ...
      if (suburbConfirmed && !suburbExact) mismatchFields.push("suburb");
      if (stateConfirmed && !stateMatch) mismatchFields.push("state");
      ...
      if (mismatchFields.length === 0) {
        // 无明确冲突：verified 或 partial
        if (unconfirmedFields.length === 0) {
          verifier.status = "verified";
          verifier.addressConfidence = "high";
        } else {
        ...
        if (vRoad && vHouseNum) {
          const unitPrefix = hasUnitInput
            ? (enteredAddress.match(/^(unit\s+\d+|apt\s+\d+|apartment\s+\d+|\d+\/)/i)?.[0] || "")
            : "";
          const streetPart = vHouseNum + " " + titleCase(vRoad);
          parts.push(unitPrefix ? unitPrefix + " " + streetPart : streetPart);
        } else {
        ...
        const failures = [];
        if (mismatchFields.includes("suburb")) {
          failures.push(`suburb: 输入 "${suburb}" 与地图 "${titleCase(vSuburb)}" 不一致`);
        }
        if (mismatchFields.includes("state")) {
          failures.push(`state: 输入 "${state}" 与地图 "${verifier.verifiedState}" 不一致`);
        }
        if (mismatchFields.includes("house_number")) {
          failures.push(`门牌号: 输入 "${inputHouseNum}" 与地图 "${vHouseNum}" 不一致`);
        }
        if (mismatchFields.includes("road")) {
          failures.push(`街道: 输入 "${inputRoad}" 未匹配地图 "${titleCase(vRoad)}"`);
        }

        const failSummary = failures.join("；");
        addressMismatch = {
          type: mismatchFields.length === 1 && mismatchFields[0] === "suburb" ? "suburb" : "comprehensive",
          inputSuburb: suburb,
          verifiedSuburb: titleCase(vSuburb),
          inputState: stateAbbrev,
          verifiedState: verifier.verifiedState,
          inputHouseNum: inputHouseNum,
          verifiedHouseNum: vHouseNum,
          inputRoad: inputRoad,
          verifiedRoad: titleCase(vRoad),
          failures: failures,
          mismatchFields: mismatchFields,
          unconfirmedFields: unconfirmedFields,
          message: `地址核验不通过：${failSummary}`
        };
