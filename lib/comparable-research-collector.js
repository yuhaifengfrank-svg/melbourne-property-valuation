...
import { hasUnitDesignator, streetAddressForGeocoding } from "./address-geocoding.js";
export { hasUnitDesignator, streetAddressForGeocoding } from "./address-geocoding.js";
...
export function addressSignature(address) {
  ...
  return {
    ...
    hasUnitSignal: /\bunit\b|\bapt\b|\bapartment\b/i.test(normalized) || /^\s*[a-zA-Z\d]+\s*\//i.test(String(address || ""))
  };
}
...
export function normalizePropertyType(type, address) {
  ...
  if (addressSignature(address).hasUnitSignal) {
    // Address has a unit designator: check normalized for apt/apartment first
    const normAddr = normalizeAddress(address);
    if (/\bapt\b|\bapartment\b/i.test(normAddr)) return "Apartment";
    return "Unit";
  }
  return "House";
}
...
export async function buildSubject(input) {
  ...
  try {
    ...
    // Handles both "unit 2/11" and bare "2/11" formats
    let queryAddress = streetAddressForGeocoding(enteredAddress);
    ...
    if (v.ok && v.results?.length > 0) {
      ...
      // Unit 检测
      const hasUnitInput = hasUnitDesignator(enteredAddress);
