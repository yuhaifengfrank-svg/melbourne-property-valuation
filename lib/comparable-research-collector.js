...
export async function verifyAddress(address) {
  // 第一步：正向地理编码
  // Nominatim 不认识 "Unit X" 前缀（如 "Unit 1, 11 McIntosh St"），
  // 去掉它才能解析出门牌号位置。但注意：坐标只用于距离衰减，
  // 不改变物业类型判定（物业类型由用户输入或下游逻辑决定）。
  const addressForNominatim = address.replace(/^(unit|apt|apartment|flat)\s+\d+[,.]?\s*/i, "").trim();
  const q = encodeURIComponent(addressForNominatim.replace(/[,.#]/g, " ").replace(/\s+/g, " ").trim());
