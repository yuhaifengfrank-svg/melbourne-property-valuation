...
export async function buildSubject(input) {
  ...
  try {
    ...
    if (v.ok) {
      ...
      // ── Suburb 精确相等（不区分大小写），拒绝子串匹配 ──
      const suburbExact = titleCase(vSuburb).toLowerCase() === suburb.toLowerCase();
      verifier.suburbExact = suburbExact;

      // 街道名匹配：忽略大小写和空格
      verifier.roadMatch = vRoad && inputRoad &&
        normalizeAddress(vRoad).toLowerCase().includes(normalizeAddress(inputRoad).toLowerCase());

      // 门牌号匹配
      verifier.houseNumMatch = vHouseNum && inputHouseNum && vHouseNum === inputHouseNum;

      // ── State 匹配：输入 state 与已验证 state 比较 ──
      const stateAbbrev = stateMap[(state || "").toLowerCase()] || state;
      const stateMatch = verifier.verifiedState && stateAbbrev && verifier.verifiedState === stateAbbrev;
      verifier.stateMatch = Boolean(stateMatch);

      // ── 综合判定：必须同时满足 suburb、state、houseNum、road ──
      const allMatch = suburbExact && stateMatch && verifier.houseNumMatch && verifier.roadMatch;
      ...
      if (allMatch) {
        ...
        if (vRoad && vHouseNum) {
          const unitPrefix = enteredAddress.match(/^(unit\s+\d+|apt\s+\d+|apartment\s+\d+|\d+\/)/i)?.[0] || "";
        ...
        const failures = [];
        if (!suburbExact) failures.push(`suburb: 输入 "${suburb}" 与地图 "${titleCase(vSuburb)}" 不一致`);
        if (!stateMatch) failures.push(`state: 输入 "${state}" 与地图 "${verifier.verifiedState}" 不一致`);
        if (!verifier.houseNumMatch) {
          failures.push(inputHouseNum
            ? `门牌号: 输入 "${inputHouseNum}" 与地图 "${vHouseNum}" 不一致`
            : `门牌号: 输入中未识别门牌号，地图返回 "${vHouseNum}"`);
        }
        if (!verifier.roadMatch) {
          failures.push(inputRoad
            ? `街道: 输入 "${inputRoad}" 未匹配地图 "${titleCase(vRoad)}"`
            : "街道: 输入中未识别街道");
        }

        const failSummary = failures.join("；");
        addressMismatch = {
          type: failures.length === 1 && !suburbExact ? "suburb" : "comprehensive",
          inputSuburb: suburb,
  ...
  const result = {
    address,
    canonicalAddress: verifier.canonicalAddress || address,
    enteredAddress,
    suburb,
    enteredSuburb,
    state,
