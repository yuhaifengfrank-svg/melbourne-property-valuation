...
export async function buildSubject(input) {
  ...
  const houseNumMatch = normalizedInput.match(/^(?:unit\s+\d+\s+)?(?:\d+\s*\/\s*)?(\d+)\s+/);
  const inputHouseNum = houseNumMatch ? houseNumMatch[1] : "";
  // 提取街道名：nDigits 之后、street suffix 之前的部分
  const roadMatch = normalizedInput.match(/(?:unit\s+\d+\s+)?(?:\d+\s*\/\s*)?\d+\s+(.+?)\s+(?:street|avenue|road|grove|drive|court|crescent|parade|place|lane|pde|rd|st|dr|crt|hwy|tce|wy|bvd|cl|ct|gdn|grn|gr|pkwy|pl|pt|sq|trc|close|circuit|gate|way|rise|view|vale|ridge|highway|motorway)/);
  ...
  try {
    ...
    if (v.ok) {
      ...
      // 判定核验结果
      const suburbOk = matchScore >= 2;
      const roadOk = verifier.roadMatch;

      if (suburbOk && roadOk) {
        // 完全核验：suburb + road 都匹配
        ...
        if (vRoad && vHouseNum) {
          // 用验证后的门牌 + 街道
          const unitPrefix = cleanStreet.match(/^(unit\s+\d+|\d+\/)/i)?.[0] || "";
      ...
      } else if (matchScore < 2 && matchScore >= 0) {
        // suburb mismatch（地址中 suburb ≠ Nominatim 结果）
        ...
        addressMismatch = {
          ...
          message: `地址中的 suburb "${suburb}" 与地图核验结果 "${titleCase(vSuburb)}" 不一致。请确认正确的 suburb。`
        };
        mismatchWarnings.push(addressMismatch.message);
      } else {
        // 部分匹配但不够确定（matchScore = 1）
        verifier.status = "partial";
        addressResolved = true;
        addressSource = "nominatim_partial";
        mismatchWarnings.push(`Suburb 核验不完全：输入 "${suburb}"，地图显示 "${titleCase(vSuburb)}"`);
      }
    } else {
      // Nominatim 不可用
