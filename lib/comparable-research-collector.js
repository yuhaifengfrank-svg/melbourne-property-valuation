...
export async function verifyAddress(address) {
  ...
  const url = `https://nominatim.openstreetmap.org/search?q=${q}&format=json&limit=10&addressdetails=1`;
  try {
        ...
        if (mismatchFields.length === 0) {
          ...
          if (vRoad && vHouseNum) {
            const unitPrefix = hasUnitInput
              ...
              : "";
            parts.push(unitPrefix ? unitPrefix + " " + vHouseNum + " " + titleCase(vRoad) : vHouseNum + " " + titleCase(vRoad));
          } else {
