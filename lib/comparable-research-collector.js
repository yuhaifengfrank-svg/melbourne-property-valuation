...
export async function buildSubject(input) {
  ...
  // ═══════════════════════════════════════════════════════════════
  try {
    // Strip unit/apt prefix before geocoding: handles
    // "Unit 2/11 McIntosh St" → "11 McIntosh St",
    // "Unit 1, 11 McIntosh St" → "11 McIntosh St",
    // "Apt 3, 45 Some St" → "45 Some St"
    let queryAddress = enteredAddress
      .replace(/^(?:unit|apt|apartment|flat)\s+\d+(?:\s*\/\s*|[,.]?\s*)/i, "")
      .replace(/^\d+\s*\//, "")
      .trim();
