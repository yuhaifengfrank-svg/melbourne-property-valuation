...
// ── Allowed clock skew for issuedAt future-check (60 seconds) ──────

const MAX_CLOCK_SKEW_MS = 60 * 1000;

/**
 * Strictly validate time fields in the decoded payload.
 *
 * Rules:
 *   1. issuedAt and expiresAt must exist, be numbers, finite, integer, > 0
 *   2. expiresAt must be > issuedAt
 *   3. expiresAt - issuedAt must be exactly TTL_MS (1800000)
 *   4. expiresAt <= Date.now() → expired (reject)
 *   5. issuedAt > Date.now() + MAX_CLOCK_SKEW_MS → future (reject)
 *
...
function validateTimeFields(payload) {
  ...
  // 1. Both must exist, be numbers, finite, integer, > 0
  if (issuedAt === undefined || issuedAt === null) return false;
  ...
  // 2. expiresAt must be strictly greater than issuedAt
  if (expiresAt <= issuedAt) return false;

  // 3. TTL must be exactly 30 minutes (1800000 ms)
  if (expiresAt - issuedAt !== TTL_MS) return false;

  // 4. Expired: expiresAt <= now
  if (expiresAt <= now) return false;

  // 5. Future: issuedAt more than clock skew ahead
...
export function verifyReportAccessSession(token, options = {}) {
  try {
    ...
    // ── Validate time fields — strict checks ───────────────────
    if (!validateTimeFields(payload)) return null;
...
/**
 * Assert that the report access session secret is configured.
 *
 * In production, this checks that REPORT_ACCESS_SESSION_SECRET is set.
 * In development/test, only a dev fallback secret is required (always available).
 *
 * @param {object} [options]
 * @param {string} [options.mockSecret]  — For testing
 * @returns {boolean}  — true if configured, false only if production && missing
 */
export function assertReportAccessSessionConfigured(options = {}) {
  const env = process.env.NODE_ENV || "development";
  const secret = options.mockSecret || process.env.REPORT_ACCESS_SESSION_SECRET;
