...
export async function getSalmBySuburb(suburb) {
  ...
  // Prefer explicitly verified multi-SA2 membership. A suburb such as Mount
  // Waverley spans both North and South and must never be reduced by LIMIT 1.
  const memberships = await q(
    `SELECT m.sa2_code, s.* FROM suburb_sa2_membership m
     JOIN salm_sa2_data s USING(sa2_code)
     WHERE LOWER(m.suburb)=LOWER($1) AND m.state='VIC' ORDER BY m.sa2_code`,
    [suburb]
  );
  if (memberships?.length) {
    const latestQuarter = memberships[0].latest_quarter;
    const baseQuarter = memberships[0].employment_growth_base_quarter;
    const sum = key => memberships.reduce((total, row) => total + Number(row[key] || 0), 0);
    const baseEmployment = memberships.reduce((total, row) => {
      const point = row.historical_data?.[baseQuarter];
      return total + (point?.lf != null && point?.unemp != null ? Number(point.lf) - Number(point.unemp) : 0);
    }, 0);
    const labourForce = sum('labour_force');
    const unemployed = sum('unemployed');
    const employment = sum('employment_count');
    const result = {
      sa2_code: memberships.map(row => row.sa2_code).join(','),
      sa2_codes: memberships.map(row => row.sa2_code),
      sa2_name: memberships.map(row => row.sa2_name).join(' + '),
      latest_quarter: latestQuarter,
      labour_force: labourForce,
      unemployed,
      unemployment_rate: memberships.length === 1 ? Number(memberships[0].unemployment_rate) : unemployed / labourForce * 100,
      employment,
      employment_count: employment,
      employment_growth_yoy: baseEmployment > 0 ? (employment / baseEmployment - 1) * 100 : null,
      employment_growth_base_quarter: baseQuarter,
      has_data: true,
      quarters_available: Math.min(...memberships.map(row => Number(row.quarters_available || 0))),
      historical_data: null,
      geography: memberships.length === 1 ? 'SA2' : 'combined SA2s',
    };
    _serviceCache.set(cacheKey, result);
    _serviceCacheTimestamps.set(cacheKey, Date.now());
    return result;
  }

  // Legacy fallback for suburbs not yet verified in suburb_sa2_membership.
...
export async function getSalmBySa2Code(sa2Code, cacheKey) {
  ...
  const salm = {
    ...
    employment: row.employment_count ?? (row.labour_force != null && row.unemployed != null
      ? row.labour_force - row.unemployed
      : null),
    employment_count: row.employment_count ?? null,
    employment_growth_yoy: row.employment_growth_yoy ?? null,
    employment_growth_base_quarter: row.employment_growth_base_quarter ?? null,
