...
import { normalizeVictoriaCoordinates } from './coordinate-policy.js';
...
export function buildPreviewQueries(previewSql, data) {
  ...
  for (const r of combinedComps) {
    const coordinates = normalizeVictoriaCoordinates(r.lat, r.lon);
    r.lat = coordinates.lat;
    r.lon = coordinates.lon;
    const cols = CS_COLS;
    const placeholders = cols.map((_, i) => `$${i + 1}`);
