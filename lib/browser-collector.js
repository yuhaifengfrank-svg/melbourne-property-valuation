...
export function parseReaSold(text, suburb) {
  ...
  const sales = [];
  // 扫描整个页面找价格行（以$开头），然后往回找地址（包含suburb的那行）
  for (let i = 0; i < lines.length; i++) {
    if (!lines[i].startsWith("$")) continue;
    const price = parseInt(lines[i].replace(/[$,]/g, ""));
    if (isNaN(price) || price < 50000 || price > 50000000) continue;

    // 往前找：包含suburb的地址行
    let address = "";
    let addressIdx = -1;
    const subLower = suburb.toLowerCase();
    for (let k = 0; k < 15 && i - k >= 0; k++) {
      const l = lines[i - k];
      if (l.startsWith("$")) continue;
      const lLower = l.toLowerCase();
      const commaIdx = lLower.lastIndexOf(",");
      if (commaIdx >= 0) {
        // 有逗号：只看逗号后的部分（地址末尾）
        const afterComma = lLower.slice(commaIdx);
        if (afterComma.includes(subLower)) {
          address = l; addressIdx = i - k; break;
        }
      } else {
        // 无逗号：整行包含完整 suburb 短语
        if (lLower.includes(subLower)) {
          address = l; addressIdx = i - k; break;
        }
      }
    }
    ...
    if (sales.some(s => s.address === address && s.price === price)) continue;

    const inferredType = inferPropertyType(address);
    const s = {
      address, price, propertyType: inferredType,
      bedrooms: null, bathrooms: null, carSpaces: null,
      landSize: null, saleDate: null, source: "realestate.com.au",
      sourceUrl: "", evidenceUrls: []
    };

    // 向前（地址行之后）找字段：bedrooms, bathrooms, carSpaces, landSize, propertyType, saleDate
    // 搜索范围：从地址行到下一个"Sold"或"Featured"或"Budgeting"广告之间
    for (let j = addressIdx + 1; j < lines.length; j++) {
      const l = lines[j];
      // 遇到下一个记录的开头或广告内容停止
      if (l === "Sold" || l.startsWith("Featured") || l.startsWith("Budgeting")) break;
      if (l.startsWith("$") && l !== lines[i]) break; // 另一个价格行
      
      if (/^\d+$/.test(l)) {
        if (s.bedrooms === null) s.bedrooms = parseInt(l);
        else if (s.bathrooms === null) s.bathrooms = parseInt(l);
        else if (s.carSpaces === null) s.carSpaces = parseInt(l);
      }
      else if (l.includes("m²")) s.landSize = parseInt(l.replace(/[^0-9]/g, ""));
      else if (/^[•·]?\s*(House|Townhouse|Apartment|Unit|Villa|Land)$/i.test(l)) {
        const matched = l.replace(/^[•·]\s*/, '').trim();
        s.propertyType = matched.charAt(0).toUpperCase() + matched.slice(1).toLowerCase();
      }
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].startsWith("$") && /^[\d,]+$/.test(lines[i].slice(1))) {
      ...
      for (let k = 0; k < 10; k++) {
        ...
        // 覆盖页面中的类型标签（优先于地址推断）
        if (l && /^(House|Townhouse|Apartment|Unit|Villa|Land)$/i.test(l)) {
          const pt = l.charAt(0).toUpperCase() + l.slice(1).toLowerCase();
          s.propertyType = pt;
        }
      }
 ...
 */
export async function scrapeSoldData(suburb, state, postcode, maxPages = 1) {
  const subSlug = suburb.toLowerCase().replace(/\s+/g, "-");
  const stateLower = state.toLowerCase();

  const allSales = [];

  // REA: 翻页 (list-1, list-2, ...)
  for (let page = 1; page <= maxPages; page++) {
    const reaListUrl = `https://www.realestate.com.au/sold/in-${subSlug}+${stateLower}/list-${page}?activeSort=solddate&propertyTypes=house`;
    try {
      const result = await fetchPageText(reaListUrl);
      if (result.ok && result.text) {
        const reaSales = parseReaSold(result.text, suburb);
        // 翻页检测：解析到 0 条成交 → 到底了
        if (reaSales.length === 0) break;
        reaSales.forEach(s => {
  ...
  // Domain: 翻页 (?page=2, ?page=3, ...)
  for (let page = 1; page <= maxPages; page++) {
    const pageParam = page === 1 ? "" : `?page=${page}`;
    const domainListUrl = postcode
      ? `https://www.domain.com.au/sold-listings/${subSlug}-${stateLower}-${postcode}/${pageParam}`
      : `https://www.domain.com.au/sold-listings/${subSlug}-${stateLower}/${pageParam}`;
    try {
      const result = await fetchPageText(domainListUrl);
      if (result.ok && result.text) {
        const domainSales = parseDomainSold(result.text, suburb);
        // 翻页检测：解析到 0 条成交 → 到底了
        if (domainSales.length === 0) break;
