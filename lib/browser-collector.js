...
// 通过 CDP WebSocket 控制真实 Chrome，绕过所有反爬
// 替换 Codex 的空骨架，支持跨州跨物业类型抓取
// 兼容 module (import) 和 CommonJS (require)
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i] === "Sold" && i + 1 < lines.length && lines[i + 1].startsWith("$")) {
      ...
      for (let j = 0; i + 3 + j < lines.length; j++) {
        const l = lines[i + 3 + j];
        if (l.startsWith("Sold") || l === "Sold" || l.startsWith("$")) break;
        ...
        else if (l.startsWith("Sold on")) s.saleDate = l.replace("Sold on ", "").trim();
      }
      sales.push(s);
    }
  }
...
export function formatAsComparables(sales, subject) {
  ...
  // 暂时只用模拟距离，后续可接入 Google Maps API
  // 距离在 formatAsComparables 调用方那里传入更合适

  return deduped
    .sort((a, b) => {
      // 按价格接近目标估价排序（目标约 $1.3m）
      const target = 1300000;
      return Math.abs(a.price - target) - Math.abs(b.price - target);
    })
    .slice(0, 12)
    .map((s, i) => {
      const d = distances.get(norm(s.address));
      ...
      return {
        ...
        salePrice: s.price,
        saleDate: s.saleDate || new Date().toISOString().slice(0, 10),
        distanceMeters: d || (i * 200 + 200),
      bedrooms: s.bedrooms || (subject.bedrooms || 3),
      bathrooms: s.bathrooms || (subject.bathrooms || 2),
      carSpaces: s.carSpaces || (subject.carSpaces || 2),
      landSize: s.landSize || (subject.landSize || 400),
      sourceUrl: s.sourceUrl || `https://${s.source}/sold/`,
      sourceCount: s.source.includes("+") ? 2 : 1,
      conditionScore: 3,
      microLocationScore: 3,
      planningScore: 3,
      riskScore: 2,
      bodyCorporateScore: 5,
