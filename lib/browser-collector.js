  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].startsWith("$") && /^[\d,]+$/.test(lines[i].slice(1))) {
      ...
      for (let k = 0; k < 10; k++) {
        ...
        // 覆盖页面中的类型标签（优先于地址推断）
        if (l && /^(House|Townhouse|Apartment|Unit|Villa|Land)$/i.test(l)) s.propertyType = l;
      }
...
export async function scrapeSoldData(suburb, state, postcode, maxPages = 1) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    try {
      ...
      if (result.ok && result.text) {
        // 翻页检测：如果文本中没有任何 sold property 结构，跳过
        if (maxPages > 1 && page > 1 && !result.text.includes("sold-property")) break;
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    try {
      ...
      if (result.ok && result.text) {
        // 翻页检测
        if (maxPages > 1 && page > 1 && !result.text.includes("listing-card")) break;
