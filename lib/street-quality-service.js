...
function streetOrientationScore(streetName) {
  ...
  if (northSouth && !eastWest) {
    // 南北向 → 两侧临街房都能朝北（临街面在东西两侧）→ 高分
    return { score: 5, orientation: "north-south" };
  }
  if (eastWest && !northSouth) {
    // 东西向 → 只有单侧房朝北 → 中等
    return { score: 3, orientation: "east-west" };
  }
  // 无明确方向 → 默认
...
export function computeStreetQuality(address, suburb, comparables = []) {
  ...
  // OSM 道路类型（weight 0.25）
  const osmScore = osmRoadClassScore(suffix);

  // 街道朝向（weight 0.15）
  const { score: orientationScore } = streetOrientationScore(street + " " + suffix);

  // 树冠绿化（weight 0.25）— 暂用默认 3，待数据就绪后启用
  const canopyScore = 3; // TODO: 从 street_tree_by_street.json 查
...
export function explainStreetQuality(address, suburb, comparables = []) {
  ...
  const components = [
    { name: "AADT交通量", weight: 0.25, score: aadtScoreVal, detail: aadtStats ? `AADT ${aadtStats.aadt_avg}` : "不在主干道=低流量" },
    { name: "街道价格水平", weight: 0.10, score: priceScoreVal, detail: `百分位 ${(pct * 100).toFixed(0)}%` },
    { name: "道路类型", weight: 0.25, score: osmScore, detail: `后缀 "${suffix}"` },
    { name: "街道朝向", weight: 0.15, score: orientationScore, detail: orientation },
    { name: "树冠绿化", weight: 0.25, score: canopyScore, detail: "暂用默认值" },
  ];
