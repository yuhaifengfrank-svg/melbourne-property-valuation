...
export function percentileScores(values, { lowerIsBetter = false } = {}) {
  ...
  for (let start = 0; start < valid.length;) {
    ...
    // Map the best observed rank to 95 and the lowest to 1 directly. Capping
    // a 0-100 percentile afterward would collapse the entire top 5% at 95.
    const score = Math.round(95 - (averageRank / denominator) * 94);
