...
export function setTokenCookie(res, token) {
  ...
  res.setHeader(
    "Set-Cookie",
    `aushomevalue_gate=${token}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=${maxAge}`
  );
}
...
export function clearTokenCookie(res) {
  res.setHeader(
    "Set-Cookie",
    `aushomevalue_gate=; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=0`
  );
}
...
export function getTokenFromCookies(req) {
  ...
  return cookies.aushomevalue_gate || null;
}
