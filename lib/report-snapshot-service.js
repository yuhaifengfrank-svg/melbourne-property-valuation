...
function stableJson(val) {
  ...
  if (Array.isArray(val)) {
    return "[" + val.map(stableJson).join(",") + "]";
  }
 ...
 *
 * Uses a single WITH (CTE) to atomically guard against concurrent double-insert:
 *  1. Checks the draft is valid, unexpired (DB-side), and unconsumed
 *  2. Tries INSERT … ON CONFLICT (draft_id) DO NOTHING so concurrent calls
 *     cannot create duplicate snapshots
 *  3. Returns existing snapshot if already consumed (idempotent)
 *  4. Marks draft consumed
 *
...
export async function consumeDraftIntoSnapshot(token, leadContactId, sql) {
  ...
  if (!validateLeadContactId(leadContactId)) {
    throw new Error("Invalid lead_contact_id: must be a positive integer");
  }
  ...
  `;
  if (drafts.length === 0) {
    throw new Error("Draft not found, already consumed, or expired");
  }
  ...
  // 7. Atomic write using CTE:
  //    - INSERT with ON CONFLICT (draft_id) DO NOTHING ensures at most one snapshot
  //    - Only then UPDATE draft as consumed
  //    - Return the snapshot report_id (existing or newly inserted)
  const result = await sql`
    ...
    SELECT COALESCE(
      (SELECT report_id FROM snap),
      (SELECT report_id FROM report_snapshots WHERE draft_id = ${draft_id})
    ) AS final_report_id
  `;

  const finalReportId = result[0]?.final_report_id;
  if (!finalReportId) {
    throw new Error("Failed to create or retrieve report snapshot");
  }

  return { report_id: finalReportId, alreadyConsumed: finalReportId !== reportId };
}
