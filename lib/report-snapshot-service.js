...
export async function consumeDraftIntoSnapshot(token, leadContactId, sql) {
  ...
  if (existing.length > 0) {
    ...
    if (
      existingRow.lead_contact_id !== null &&
      existingRow.lead_contact_id !== undefined &&
      Number(existingRow.lead_contact_id) !== Number(leadContactId)
    ) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (laterResult.length > 0 && laterResult[0].report_id) {
      ...
      if (
        lr.lead_contact_id !== null &&
        lr.lead_contact_id !== undefined &&
        Number(lr.lead_contact_id) !== Number(leadContactId)
      ) {
        throw new DraftTokenError(
          "This report already belongs to another customer.",
          "REPORT_OWNER_CONFLICT"
