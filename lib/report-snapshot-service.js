 ...
 * (which was installed at Phase 1C-4B).  Reuses the same env var to keep
 * secret management simple — there's no harm in sharing the signing key
 * across different token types, as long as payloads are structured distinct
 * so a cookie token can't pass as a draft token.
 *
 * You can override with STRIPE_WEBHOOK_SECRET as a fallback for legacy
 * deployments.  After everyone's migrated to TOKEN_SIGNING_SECRET, remove
 * the fallback.
 */
function getDraftSecret() {
  const primary = process.env.TOKEN_SIGNING_SECRET;
  if (primary) return primary;
  // Legacy deployments that don't yet have TOKEN_SIGNING_SECRET
  return process.env.STRIPE_WEBHOOK_SECRET || "dev-draft-secret";
}

/**
 * Generate a draft_id: {epoch_ms}_{8 random hex}.
 */
...
/**
 * Deterministic JSON.stringify — same input always produces same output.
 * Keys are sorted alphabetically. If value is already a string, it is
 * parsed first to normalise key ordering (simulates DB JSONB behaviour).
 *
 * @param {any} input
 * @returns {string}
 */
...
/**
 * SHA-256 hash of snapshot content for integrity verification.
...
function validateLeadContactId(id) {
  ...
  // Accept number, string, or bigint that represents a positive integer
  if (typeof id === "number" && Number.isInteger(id) && id > 0) return true;
  if (typeof id === "string") {
    const n = Number(id);
    if (Number.isInteger(n) && n > 0) return true;
  }
  // @neondatabase/serverless returns bigint for BIGSERIAL columns
  if (typeof id === "bigint" && id > 0) return true;
  return false;
}
 ...
 *  1. Verify token, check existing snapshot (fast path, may already be consumed).
 *  2. Read draft from DB (DB-side expiry check).
 *  3. Verify hash integrity.
 *  4. Transaction: re-check within TX, then INSERT (avoids partial-UNIQUE
 *     index limitation with PostgreSQL ON CONFLICT).
 *  5. Mark draft consumed.
 *  6. Return { report_id, alreadyConsumed }.
 *
...
export async function consumeDraftIntoSnapshot(token, leadContactId, sql) {
  ...
  // 7. INSERT without ON CONFLICT — the partial UNIQUE index (WHERE draft_id IS NOT NULL)
  //    is incompatible with PostgreSQL ON CONFLICT. We already checked in Step 2 that no
  //    snapshot exists for this draft_id; if a concurrent caller snipes us, the partial
  //    UNIQUE index on draft_id will reject the duplicate INSERT.
