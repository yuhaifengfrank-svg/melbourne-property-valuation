...
export class DatabaseComparableSource {
  ...
  /**
   * 清除可用缓存，强制下次 checkConnection 重试
   */
  resetConnection() {
    this._available = false;
  }
  ...
  async fetch(subject, { maxResults = 12, maxAgeDays = 730 } = {}) {
    ...
    // Keep the full candidate pool until price and distance validation has run.
    // Truncating here lets recent bad rows crowd out older usable evidence.
    rows = prioritizeSaleRows(deduplicateSaleRows(rows));
    ...
    const pricesWithData = sameTypeRows.filter(r => r.sale_price > 100000).map(r => Number(r.sale_price));
    if (pricesWithData.length > 0) {
      pricesWithData.sort((a, b) => a - b);
      const mid = Math.floor(pricesWithData.length / 2);
      referencePrice = pricesWithData.length % 2 === 0
        ? (pricesWithData[mid - 1] + pricesWithData[mid]) / 2
        : pricesWithData[mid];
    }

    const rowsBefore = rows.length;
    // 价格距离联合过滤：先严格过滤（3x上下界），如果剩余不足 maxResults 条，
    // 则放宽到 5x 再次尝试。避免只拿到最新高端成交导致全部被砍。
    let priceLow = referencePrice > 0 ? referencePrice * 0.3 : 0;
    let priceHigh = referencePrice > 0 ? referencePrice * 3 : Infinity;
    let filtered = rows.filter(r => {
      if (!r.sale_price || r.sale_price <= 50000) return false;
      if (priceLow > 0 && priceHigh < Infinity) {
        if (r.sale_price < priceLow || r.sale_price > priceHigh) return false;
      }
      if (hasUsableCoordinates(subLat, subLon) && hasUsableCoordinates(r.lat, r.lon)) {
        const dist = distanceBetween(subLat, subLon, Number(r.lat), Number(r.lon));
        if (dist > MAX_DISTANCE_M) return false;
      }
      return true;
    });
    // 如果严格过滤后不足 maxResults，放宽价格限制到 5x
    if (filtered.length < maxResults && priceLow > 0 && priceHigh < Infinity) {
      priceLow = referencePrice * 0.2;
      priceHigh = referencePrice * 5;
    ...
    rows = deduplicateSaleRows(filtered, maxResults);
    ...
    return rows
      ...
      distanceMeters: (hasUsableCoordinates(subLat, subLon) && hasUsableCoordinates(r.lat, r.lon))
...
export function hasUsableCoordinates(lat, lon) {
  const numericLat = Number(lat);
  const numericLon = Number(lon);
  return Number.isFinite(numericLat) && Number.isFinite(numericLon)
    && !(numericLat === 0 && numericLon === 0);
}

export function prioritizeSaleRows(rows) {
  return [...rows].sort((a, b) => {
    const verificationDelta = verificationRank(b.verification_status) - verificationRank(a.verification_status);
...
const TYPE_COMPAT_BROADEN = {
  ...
  // Unit/Townhouse/Villa 边界模糊，数据源常混标，因此受控互通。
...
async function matchSuburbType(sql, suburb, state, type, limit, maxAgeDays) {
  ...
  if (compatTypes && compatTypes.size > 1) {
    ...
    return sql`
      ...
      ORDER BY
        (verification_status IN ('cross_source_verified', 'verified', 'single_source_observed')) DESC,
