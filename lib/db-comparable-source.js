...
const VIC_COORDINATE_BOUNDS = Object.freeze({
  minLat: -39.3,
  maxLat: -33.9,
  minLon: 140.8,
  maxLon: 150.1,
...
export function isValidVictoriaCoordinatePair(lat, lon) {
  ...
  return Number.isFinite(latitude) && Number.isFinite(longitude)
    && latitude >= VIC_COORDINATE_BOUNDS.minLat
    && latitude <= VIC_COORDINATE_BOUNDS.maxLat
    && longitude >= VIC_COORDINATE_BOUNDS.minLon
    && longitude <= VIC_COORDINATE_BOUNDS.maxLon;
}

function sameSuburb(a, b) {
  return String(a || "").trim().toLowerCase() === String(b || "").trim().toLowerCase();
}

export function comparableDistancePolicy(subject, comparable) {
  const sameSuburbMatch = sameSuburb(subject?.suburb, comparable?.suburb);
  const subjectHasCoordinates = isValidVictoriaCoordinatePair(
    subject?.coordinates?.lat,
    subject?.coordinates?.lon,
  );
  const comparableHasCoordinates = isValidVictoriaCoordinatePair(comparable?.lat, comparable?.lon);

  if (!sameSuburbMatch && (!subjectHasCoordinates || !comparableHasCoordinates)) {
    return { keep: false, distanceMeters: null, status: "cross_suburb_distance_unavailable" };
  }
  if (!subjectHasCoordinates || !comparableHasCoordinates) {
    return { keep: true, distanceMeters: null, status: "same_suburb_distance_unavailable" };
  }
  return {
    keep: true,
    distanceMeters: Math.round(distanceBetween(
      Number(subject.coordinates.lat),
      Number(subject.coordinates.lon),
      Number(comparable.lat),
      Number(comparable.lon),
    )),
    status: "measured",
  };
}
...
export class DatabaseComparableSource {
  ...
  async fetch(subject, { maxResults = 24, maxAgeDays = 365 } = {}) {
    ...
    return rows
      ...
      .map(r => ({ row: r, distance: comparableDistancePolicy(subject, r) }))
      .filter(({ distance }) => distance.keep)
      .map(({ row: r, distance }) => ({
...
async function fetchLargeLotComparables(subject, options = {}) {
  ...
  const coordinatesValid = isValidVictoriaCoordinatePair(coordinates?.lat, coordinates?.lon);
  const subLat = coordinatesValid ? Number(coordinates.lat) : null;
  const subLon = coordinatesValid ? Number(coordinates.lon) : null;
...
function formatRows(rows, subjCoords = {}) {
  const subjectCoordinatesValid = isValidVictoriaCoordinatePair(subjCoords.lat, subjCoords.lon);
  const sLat = subjectCoordinatesValid ? Number(subjCoords.lat) : null;
  const sLon = subjectCoordinatesValid ? Number(subjCoords.lon) : null;
  return rows.map(r => {
    // Compute distance
    let dist = null;
    if (subjectCoordinatesValid && isValidVictoriaCoordinatePair(r.lat, r.lon)) {
    ...
    return {
      ...
      distanceMeters: dist,
      distanceStatus: dist == null ? "same_suburb_distance_unavailable" : "measured",
      ageMonths: ageM,
