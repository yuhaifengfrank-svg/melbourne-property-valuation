...
export class DatabaseComparableSource {
  ...
  async fetch(subject, { maxResults = 12, maxAgeDays = 730 } = {}) {
    ...
    if (rows.length < 3) {
      const c1 = await matchSuburb(sql, suburb, state, maxResults, maxAgeDays);
      rows = mergeUnique(rows, c1, maxResults);
    }
    if (rows.length < 3 && postcode) {
      const c2 = await matchPostcode(sql, postcode, state, maxResults, maxAgeDays);
      rows = mergeUnique(rows, c2, maxResults);
    }
    if (rows.length < 3 && sa2Code) {
      const c3 = await matchSa2(sql, sa2Code, maxResults, maxAgeDays);
