...
/**
 * 类型兼容性映射
 * Unit 查询应匹配 DB 中 property_type 为 Unit/Apartment/Townhouse/Villa 的记录
 */
const TYPE_COMPAT_BROADEN = {
  Unit: new Set(['Unit', 'Apartment', 'Townhouse', 'Villa']),
...
async function matchSuburbType(sql, suburb, state, type, limit, maxAgeDays) {
  ...
  // 兼容类型扩展：如果是 Unit 查询，也取 Apartment 等兼容类型
  ...
  if (compatTypes && compatTypes.size > 1) {
    ...
    return sql`
      ...
      WHERE suburb ILIKE ${suburb}
        AND state = ${state}
        AND property_type IN (${sql(typesArr)})
 ...
 */
/**
 * 生成 property_type 过滤条件，支持兼容类型扩展（Unit 查也取 Apartment 等）
 */
function typeFilter(sql, type) {
  ...
  if (compat && compat.size > 1) {
    const arr = [...compat];
    return sql`AND property_type IN (${sql(arr)})`;
  }
