...
export async function runValuation(params, options = {}) {
  ...
  const parsedAddress = subjectFromCollector.parsedAddress || address;
  ...
  if (comps.length < 3 && state && (useDatabaseFallback || allowFetch === false)) {
    ...
    if (dbAvailable) {
      // 传递 postcode、sa2Code（如果能从地址解析出）
      const postcodeMatch = (parsedAddress && parsedAddress.match(/\b(\d{4})\b/)) || (address && address.match(/\b(\d{4})\b/));
      ...
      const dbRecs = await dbSource.fetch({
        ...
        sa2Code: ""  // 暂不映射 SA2；提前提取需要 SA2 映射表
      }, { maxResults });
      if (dbRecs.length > 0) {
