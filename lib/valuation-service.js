...
export async function runValuation(params, options = {}) {
  ...
  // scores this pool before retaining the final 12 comparables. Keeping the
  // SQL limit at 12 would make recency the de-facto selection algorithm and
  // prevent distance/type/accommodation quality from influencing selection.
  const finalComparableLimit = 12;
  const databaseCandidateLimit = 60;
  // 可注入的 DB source（测试用）
  ...
  if (options.mockCollectorComparables && Array.isArray(options.mockCollectorComparables)) {
    ...
    if (dbAvailable) {
      ...
      }, { maxResults: databaseCandidateLimit });
      if (dbRecs.length > 0) {
        comps = dbRecs.map(r => ({
          ...r,
  ...
  if (acc.length >= 3 && (landSize || autoResolvedLandSize)) {
    try {
      ...
      const dbUrl = process.env.DATABASE_URL || process.env.PREVIEW_DATABASE_URL || "";
