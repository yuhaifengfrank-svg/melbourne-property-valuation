...
import { classifyUnitDesignator } from "./address-geocoding.js";
...
import { assertDatabaseEnvironment } from "./database-environment.js";
...
export async function runValuation(params, options = {}) {
  ...
  // Fetch a broad candidate pool from the database. The valuation engine
  // scores this pool before retaining the final 24 comparables. Keeping the
  // SQL limit at 24 would make recency the de-facto selection algorithm and
  // prevent distance/type/accommodation quality from influencing selection.
  const finalComparableLimit = 24;
  const databaseCandidateLimit = 120;
  const standardLookbackDays = 365;
  // 可注入的 DB source（测试用）
  ...
  if (options.mockCollectorComparables && Array.isArray(options.mockCollectorComparables)) {
    ...
    if (dbAvailable) {
      ...
      }, { maxResults: databaseCandidateLimit, maxAgeDays: standardLookbackDays });
      // If fewer than 3 qualified records in 365 days, widen to 730.
      if (dbRecs.length < 3) {
        const wider = await dbSource.fetch({
          suburb: effectiveSuburb,
          state: effectiveState,
          propertyType: propertyType,
          postcode: postcodeFromCollector,
        ...
        }, { maxResults: databaseCandidateLimit, maxAgeDays: 730 });
        dbRecs = wider;
      }
      if (dbRecs.length > 0) {
        comps = dbRecs.map(r => ({
    ...
    // ── DB 不足时，用 CDP 实时数据补充 ──
    // Only supplement a genuinely small database pool. The engine performs
    // the final quality-ranked selection, so a full candidate pool must not
    // be truncated here.
    if (comps.length < finalComparableLimit && allowFetch !== false) {
      ...
      if (cdpComps.length > 0) {
        ...
        for (const c of cdpComps) {
          if (!existingAddr.has(c.address) && comps.length < databaseCandidateLimit) {
  ...
  if (acc.length >= 3 && (landSize || autoResolvedLandSize)) {
    try {
      ...
      const dbUrl = assertDatabaseEnvironment();
      ...
      for (const c of acc) {
        ...
        let dKm = null;
        const subjectCoordinatesValid = Number.isFinite(Number(subjLat)) && Number.isFinite(Number(subjLon))
          && Number(subjLat) >= -39.3 && Number(subjLat) <= -33.9
          && Number(subjLon) >= 140.8 && Number(subjLon) <= 150.1;
        const comparableCoordinatesValid = Number.isFinite(m?.lat) && Number.isFinite(m?.lon)
          && m.lat >= -39.3 && m.lat <= -33.9
          && m.lon >= 140.8 && m.lon <= 150.1;
        if (subjectCoordinatesValid && comparableCoordinatesValid) {
        ...
        const distanceWeight = dKm == null ? 0.25 : _dw(dKm);
        const cw = distanceWeight * 0.65 + _ddw(days) * 0.20 + lr * 0.15;
        const ap = Math.round(c.salePrice * lr);
        processed.push({
          ap, cw, addr: c.address, land: c.landSize,
          distKm: dKm == null ? null : Math.round(dKm * 100) / 100,
          distanceStatus: dKm == null ? "unknown" : "measured",
        });
      }
      ...
      console.warn("[runValuation] new model unavailable");
    }
  }
...
function overridePropertyTypeFromAddress(address, incomingType) {
  // ── B: incomingType 为空时，从 address 字符串推断类型 ────────────
  if (!address) return "House";
  if (!incomingType) {
    const classified = classifyUnitDesignator(address);
    if (classified === "Apartment") return "Apartment";
    if (classified === "Unit") return "Unit";
    return "House";
  }
  ...
  const hasUnitSignal = /\bunit\s+\d+\b|\bunit\b|[a-z\d]+\s*\/|\bflat\b|^[a-z\d]+\s*\//.test(normalized);
