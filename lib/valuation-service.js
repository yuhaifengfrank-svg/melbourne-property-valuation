...
async function fetchMedianForProperty(suburb, state, propertyType, dbSource) {
  try {
    ...
    // Fallback 2: suburb_metrics.median_house_price / median_unit_price
    try {
      const col = propertyType === 'house' ? 'median_house_price' : 'median_unit_price';
      const [fallbackRow] = await sql\`
        SELECT \${sql(col)} as median_price
        FROM suburb_metrics
        WHERE LOWER(suburb) = LOWER(\${suburb}) AND state = \${state}
          AND \${sql(col)} IS NOT NULL
        LIMIT 1
      \`;
      if (fallbackRow && fallbackRow.median_price) {
        return { sale_count: 0, median_price: Number(fallbackRow.median_price), q1_price: null, q3_price: null, median_bedrooms: null, median_bathrooms: null, median_land_size: null };
      }
    } catch (_) {}

    // Fallback 3: SA2 / state-wide median from comparable_sales
    try {
      const [stateRow] = await sql\`
        SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY sale_price)::bigint as median_price
        FROM comparable_sales
        WHERE state = \${state}
          AND property_type ILIKE \${propertyType}
          AND sale_price IS NOT NULL
          AND sale_date >= CURRENT_DATE - INTERVAL '3 years'
      \`;
      if (stateRow && stateRow.median_price) {
        return { sale_count: 0, median_price: Number(stateRow.median_price), q1_price: null, q3_price: null, median_bedrooms: null, median_bathrooms: null, median_land_size: null };
      }
    } catch (_) {}
