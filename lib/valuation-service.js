 ...
 * @param {*} [options.sql] 可注入的 SQL client（Heritage 查询/测试用）
...
export async function runValuation(params, options = {}) {
  ...
  if (heritageCoords?.lat && heritageCoords?.lon) {
    try {
      const db = options.sql || (await import('../api/_db.js')).getSql();
    ...
    valuation: hasEstimate
      ? (() => {
          ...
          if (heritageResult.flagged && est.midpoint) {
            const discount = 0.20;
            const discounted = (value) => value != null && value !== "" && Number.isFinite(Number(value))
              ? Math.round(Number(value) * (1 - discount))
              : value;
            est.midpoint = discounted(est.midpoint);
            est.low = discounted(est.low);
            est.high = discounted(est.high);
            est.anchor = discounted(est.anchor);
            est.weightedMedian = discounted(est.weightedMedian);
            est.weightedMean = discounted(est.weightedMean);
...
function overridePropertyTypeFromAddress(address, incomingType) {
  ...
  // A hyphen can mean either a unit shorthand (2-11 => 2/11) or a street
  // number range (5-7). Only treat it as a unit when the selected property
  // type supplies that context.
  const hyphenMeansUnit = ["Unit", "Townhouse", "Villa"].includes(incomingType);
  const prepped = address.toLowerCase()
    .replace(/[,\.]/g, " ")
    .replace(/\b(\d+)\s*-\s*(\d+)\b/g, hyphenMeansUnit ? "$1/$2" : "$1-$2")
