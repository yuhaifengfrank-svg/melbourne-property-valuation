...
export async function runValuation(params, options = {}) {
  ...
  return {
    ...
    sourceResults: collectorResult.sourceResults || [],
    // evidenceMode 标准：必须来源可达且至少 3 个被接受的 comps
    // 否则标记为 database_verified 或 unavailable
    evidenceMode: (() => {
      if (!hasEstimate) return "unavailable";
      if (!allowFetch) return "database_verified";
      // 有实时数据时，检查是否有足够证据
      ...
      if (acc.length > 0) return "database_verified";
      return "unavailable";
    })(),
    isFallback: !hasEstimate,
    collectedAt: new Date().toISOString(),
    asOfDate: new Date().toISOString().slice(0, 10)
  };
}
