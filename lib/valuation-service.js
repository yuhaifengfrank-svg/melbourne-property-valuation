...
export async function runValuation(params, options = {}) {
  ...
  // ── 学校因子：在估值后注入学校评分到 subject ──
  // 学校平均 ICSEA 作为环境指标，由 valuation-engine 内部决定修正值
  if (effectiveSuburb) {
    const schoolData = await getSuburbSchoolProfile(effectiveSuburb, effectiveState);
    if (schoolData && schoolData.avgIcsea != null) {
      subj.avgIcsea = schoolData.avgIcsea;
      subj.schoolCount = schoolData.schoolCount;
    }
  }
  ...
  return {
    ...
    customerDataStatus: (() => {
      ...
      const crossVerified = comps.filter(c => c.verificationStatus === "cross_source_verified").length;
      const singleObserved = comps.filter(c => c.verificationStatus === "single_source_observed").length;
