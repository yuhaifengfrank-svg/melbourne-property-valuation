...
export async function runValuation(params, options = {}) {
  ...
  return {
    ...
    sourceResults: collectorResult.sourceResults || [],
    isSingleSource: (() => {
      const ssc = acc.filter(c =>
        c._sourceMode === "database_verified" && c.verificationStatus === "single_source_observed"
      );
      return ssc.length >= 5;
    })(),
    // evidenceMode 标签含义：
    //   live_verified        — 实时 CDP 抓取 + 至少 3 个被接受 comps
    //   database_single_source — 从数据库读取单来源观测记录
    //   database_verified    — 从数据库读取已验证记录（跨来源）
    //   research_only        — 有一些 comps 但来源不够充分
    //   unavailable          — 无法获取任何 data
    evidenceMode: (() => {
      if (!hasEstimate) return "unavailable";
      // 数据库来源：只接受同时标记了 DB 来源且状态为 verified 的记录
      if (primaryMode === "database_verified") {
        const acceptedCrossSource = acc.filter(c =>
          c._sourceMode === "database_verified" && c.verificationStatus === "cross_source_verified"
        );
        const acceptedSingleSource = acc.filter(c =>
          c._sourceMode === "database_verified" && c.verificationStatus === "single_source_observed"
        );
        if (acceptedCrossSource.length >= 1) return "database_verified";
        if (acceptedSingleSource.length >= 5) return "database_single_source";
