...
export async function runValuation(params, options = {}) {
  ...
  // 估值（数据已注入）
  // ── Large-Lot Detection: prep land stats ──
  let largeLotLandStats = null;
  if (effectiveSuburb && propertyType === "House") {
    try {
      ...
      largeLotLandStats = { median: med?.median_area_m2 || null, p90 };
    } catch { /* silent */ }
  }

  // Query large-lot comparables from DB if not already injected
  const largeLotComps = options.largeLotComparables || [];
  if (largeLotLandStats && !largeLotComps.length && effectiveState) {
    try {
  ...
  const valuation = valueProperty({
    ...
    subject: {
      ...
      landSizeSource: subj.landSizeSource || (autoResolvedLandSize ? "lga_residential_proxy" : null),
