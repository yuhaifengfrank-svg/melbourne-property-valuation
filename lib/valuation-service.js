...
export async function runValuation(params, options = {}) {
  ...
  if (comps.length < 3 && state && (useDatabaseFallback || allowFetch === false)) {
    ...
    if (dbAvailable) {
      // 使用 collector 已解析的 postcode（Nominatim 可找到地址邮编），而不是纯正则
      const postcodeFromCollector = subjectFromCollector.postcode || "";
      // 从 collector 解析或原始地址正则提取 sa2_code（目前暂不实现 SA2 映射）
      // SA2 映射表实现后在 db-comparable-source.fetch 内读取
  ...
  return {
    ...
    evidenceMode: (() => {
      ...
      // 数据库来源：只检查确实被估值引擎接受的记录
      if (primaryMode === "database_verified") {
        const acceptedFromDb = acc.filter(c => c.verificationStatus === "verified" || c._sourceMode === "database_verified");
        if (acceptedFromDb.length >= 1) return "database_verified";
