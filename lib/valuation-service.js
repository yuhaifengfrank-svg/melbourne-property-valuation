...
export async function runValuation(params, options = {}) {
  ...
  const effectiveAddress = preserveUnitAddress(address, collectorSubject.canonicalAddress || address, propertyType);
  ...
  if (options.mockCollectorComparables && Array.isArray(options.mockCollectorComparables)) {
    ...
    if (dbAvailable) {
      ...
      const dbRecs = await dbSource.fetch({
        ...
        internalArea: params?.internalArea ? Number(params.internalArea) : null,
        buildingArea: params?.buildingArea ? Number(params.buildingArea) : null,
 ...
 */
export function overridePropertyTypeFromAddress(address, incomingType) {
  ...
  const explicitUnitSignal = /\bunit\s+\d+\b|\bunit\b|[a-z\d]+\s*\/|\bflat\b|^[a-z\d]+\s*\//.test(prepped);
  // A hyphen is ambiguous: Unit users commonly enter 2-11, while House
  // addresses such as 5-7 are street-number ranges. Respect the selected type.
  const hyphenUnitSignal = (incomingType === "Unit" || incomingType === "Apartment")
    && /^\s*[a-z\d]+\s*-\s*\d+[a-z]?\b/i.test(address);
  const hasUnitSignal = explicitUnitSignal || hyphenUnitSignal;
...
export function preserveUnitAddress(inputAddress, canonicalAddress, propertyType) {
  if (propertyType !== "Unit" && propertyType !== "Apartment") return canonicalAddress;
  const match = String(inputAddress || "").match(/^\s*([a-z\d]+)\s*[\/-]\s*(\d+[a-z]?)\b/i);
  if (!match) return canonicalAddress;
  if (/^\s*[a-z\d]+\s*\//i.test(String(canonicalAddress || ""))) return canonicalAddress;
  const canonical = String(canonicalAddress || inputAddress).replace(/^\s*\d+[a-z]?\b\s*/i, "");
  return `${match[1]}/${match[2]} ${canonical}`.replace(/\s+/g, " ").trim();
}
