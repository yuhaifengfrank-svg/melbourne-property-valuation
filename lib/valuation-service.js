...
async function fetchMedianForProperty(suburb, state, propertyType, dbSource) {
  try {
    ...
    // Fallback 2: suburb_metrics.median_price
    try {
      const isHouse = propertyType === 'house';
      const [fb] = await sql\`
        SELECT
          CASE WHEN \${isHouse} THEN median_house_price ELSE median_unit_price END as median_price
        FROM suburb_metrics
        WHERE LOWER(suburb) = LOWER(\${suburb}) AND state = \${state}
          AND CASE WHEN \${isHouse} THEN median_house_price IS NOT NULL ELSE median_unit_price IS NOT NULL END
        LIMIT 1
      \`;
