...
export function buildFactorBreakdown(suburb) {
  ...
  return {
    ...
    lga: suburb.lga_name ?? null,
    sa2Code: suburb.sa2_code ?? null,

    // ── Extra raw metrics not in factor scores ──
    // medianApartmentPrice temporarily hidden — data quality issue in Glen Waverley
    medianTownhousePrice: Number(suburb.median_townhouse_price) ?? null,
    medianCombinedPrice: Number(suburb.median_combined_price) ?? null,
    medianHouseRent: Number(suburb.median_rent) ?? null,
    medianHouseRentDffh: Number(suburb.median_rent_dffh) ?? null,
    medianUnitRent: Number(suburb.median_unit_rent) ?? null,
    vacancyRateAdjusted: Number(suburb.vacancy_rate_adjusted) ?? null,

    // Population (absolute + projection)
    population2021: Number(suburb.population_2021) ?? null,
    population2025: Number(suburb.population_2025) ?? null,
    populationCagr5y: Number(suburb.population_cagr_5y) ?? null,
    populationGrowth1y: Number(suburb.population_growth_1y) ?? null,
    populationProjection2036: Number(suburb.population_projection_2036) ?? null,
    populationScore: Number(suburb.population_score) ?? null,

    // Undervaluation
    undervaluation: Number(suburb.undervaluation) ?? null,
    undervaluationLabel: suburb.undervaluation_label ?? null,

    // Supply chain & employment
    supplyUnemploymentRate: Number(suburb.supply_unemployment_rate) ?? null,
    supplyEmploymentGrowth: Number(suburb.supply_employment_growth) ?? null,

    // Dwelling structure
    dwellingTotal: Number(suburb.dwelling_total) ?? null,
    dwellingSeparateHouse: Number(suburb.dwelling_separate_house) ?? null,
    dwellingFlat: Number(suburb.dwelling_flat) ?? null,
    dwellingSemiDetached: Number(suburb.dwelling_semi_detached) ?? null,
    dwellingOccupancyRate: Number(suburb.dwelling_occupancy_rate) ?? null,

    // Confidence sub-scores
    confValue: Number(suburb.conf_value) ?? null,
    confGrowth: Number(suburb.conf_growth) ?? null,
    confYield: Number(suburb.conf_yield) ?? null,
    confVacancy: Number(suburb.conf_vacancy) ?? null,
    confSchool: Number(suburb.conf_school) ?? null,
    confIncome: Number(suburb.conf_income) ?? null,
...
export async function fetchSuburbIntelligence(suburbName) {
  const rows = await q(
    `SELECT sm.*, sl.postcode, sl.lga_name, sl.sa2_code
     FROM suburb_metrics sm
     LEFT JOIN (SELECT DISTINCT ON (LOWER(suburb)) LOWER(suburb) AS lsuburb, postcode, lga_name, sa2_code
                FROM school_locations WHERE state = 'VIC') sl
       ON LOWER(sm.suburb) = sl.lsuburb
     WHERE LOWER(sm.suburb) = LOWER($1)`,
    [suburbName]
  );
    ...
    const fuzzy = await q(
      `SELECT sm.*, sl.postcode, sl.lga_name, sl.sa2_code
       ...
       LEFT JOIN (SELECT DISTINCT ON (LOWER(suburb)) LOWER(suburb) AS lsuburb, postcode, lga_name, sa2_code
                  FROM school_locations WHERE state = 'VIC') sl
         ON LOWER(sm.suburb) = sl.lsuburb
       WHERE LOWER(sm.suburb) LIKE LOWER($1) LIMIT 1`,
      [`%${suburbName}%`]
    );
