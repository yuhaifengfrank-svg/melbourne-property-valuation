...
function scoreSchool(suburb) {
  const s = Number(suburb.school_score) || null;
  if (s == null) return { score: 40, tier: 'C' };
...
function explainSchool(s, suburb) {
  ...
  if (ss != null) {
    ...
    if (ss >= 60 && ss < 80) lines.push(`School score of ${ss} is above average — adds moderate family-demand premium`);
    if (ss >= 40 && ss < 60) lines.push(`School score of ${ss} is average — adequate schooling available`);
    if (ss < 40) lines.push(`School score of ${ss} is below average — family buyers may be less attracted to this area`);
...
export function buildFactorBreakdown(suburb) {
  ...
  return {
    ...
    // Factor scores
    value: { score: value.score, tier: value.tier, raw: value.raw ?? null },
    ...
    school: { score: school.score, tier: school.tier, raw: Number(suburb.school_score) ?? null },
...
export async function fetchTopByFactor(factor, limit = 20) {
  // Map factor names to DB columns for sorting
  const scoreMap = {
    value: 'median_house_price ASC',          // cheaper = better value
    growth: 'GREATEST(COALESCE(growth_3y,0), COALESCE(growth_5y,0)) DESC',
    yield: 'gross_yield DESC NULLS LAST',
    school: 'school_score DESC NULLS LAST',
  };

  const orderBy = scoreMap[factor];
  if (!orderBy) throw new Error(`Unknown factor: ${factor}`);

  const rows = await q(
    `SELECT * FROM suburb_metrics WHERE opportunity_score IS NOT NULL ORDER BY ${orderBy} LIMIT ${Math.min(limit, 50)}`
  );

  // We need to score them properly after fetching raw data
  const scored = rows.map(r => {
    ...
    return {
      suburb: fb.suburb,
      state: fb.state,
      factorScore: fb[factor]?.score ?? null,
      factorTier: fb[factor]?.tier ?? null,
      ...
      explanations: fb.explanations[factor].slice(0, 3), // Top 3 explanations
    };
  });
