...
import { percentileScores } from './relative-score.js';
...
function scoreYield(suburb) {
  const y = Number(suburb.gross_yield_house_3br ?? suburb.gross_yield_house_4br) || null;
  if (y == null) {
    return { score: null, tier: 'N/A', unavailable: true };
  }
...
function scoreVacancy(suburb) {
  const kind = suburb.rental_vacancy_kind;
  const v = Number(suburb.rental_vacancy_estimate);
  if (!['fact', 'estimate'].includes(kind) || !Number.isFinite(v)) {
    return { score: null, tier: 'N/A', unavailable: true };
  }
...
function explainGrowth(s, suburb) {
  const g1 = numberOrNull(suburb.growth_1y);
  const g3 = numberOrNull(suburb.growth_3y);
  const g5 = numberOrNull(suburb.growth_5y);
  const lines = [];
  const signals = [g1, g3, g5].filter(Number.isFinite);
  if (signals.length) {
    lines.push('Experimental sales-momentum signals contribute to this relative rank. They are not measured 1-, 3- or 5-year returns or CAGR.');
  } else lines.push('Historical growth is unavailable until an official dated price series passes publication checks');
  return lines;
}

function explainYield(s, suburb) {
  const y = numberOrNull(suburb.gross_yield_house_3br ?? suburb.gross_yield_house_4br);
...
function explainVacancy(s, suburb) {
  const v = ['fact', 'estimate'].includes(suburb.rental_vacancy_kind)
    ? numberOrNull(suburb.rental_vacancy_estimate) : null;
  const lines = [];
  if (v != null) {
...
function explainSupply(s, suburb) {
  ...
  const emp = Number(suburb.supply_employment_count);
  const unemp = Number(suburb.supply_unemployment_rate);
...
export function buildFactorBreakdown(suburb) {
  ...
  return {
    ...
    yield: { score: yield_.score, tier: yield_.tier, raw: yield_.raw ?? null, unavailable: yield_.unavailable || false },
    vacancy: { score: vacancy.score, tier: vacancy.tier, raw: vacancy.unavailable ? null : Number(suburb.rental_vacancy_estimate), unavailable: vacancy.unavailable || false },
    school: {
      score: school.score,
      tier: school.tier,
...
export async function fetchTopByFactor(factor, limit = 20) {
  ...
  // Rank raw factor signals across the full population, then publish 5-point bands.
  const breakdowns = rows.map(r => buildFactorBreakdown(r));
  const rawValues = rows.map(row => rawFactorSignal(row, factor));
  const relativeScores = percentileScores(rawValues, { lowerIsBetter: factor === 'value' });
  const scored = rows.map((r, index) => {
    const fb = breakdowns[index];
    const fs = fb[factor];
    return {
      suburb: fb.suburb,
      state: fb.state,
      factorScore: relativeScores[index],
...
function rawFactorSignal(row, factor) {
  const finite = value => value === null || value === undefined || value === ''
    ? null
    : Number.isFinite(Number(value)) ? Number(value) : null;
  if (factor === 'value') return finite(row.median_house_price) ?? finite(row.median_unit_price);
  if (factor === 'yield') return finite(row.gross_yield);
  if (factor === 'school') return finite(row.school_score);
  if (factor === 'supply') {
