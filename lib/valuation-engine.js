...
function scoreComparable(subject, comp, asOfDate) {
  ...
  const srcCount = Math.max(1, Number(comp.sourceCount) || (comp.crossChecked ? 2 : 1));
    ...
    const scores = {
    propertyType: subjectType === compType ? 18 : typeMatches(subjectType, compType) ? 10 : 0,
...
export function channelAEstimate(subject, comparables, landElasticity = 0.196) {
  ...
  const adjusted = comparables.map(c => {
    ...
    // Time adjustment (bidirectional: recent = premium, old = discount)
    // ageM is already in months; use gentle 0.3%/month slope with 24mo inflection
    const ageM = number(c.ageMonths) || 0;
    const timeAdj = 1.0 - Math.min(0.10, ageM * 0.003);

    // Non-linear land adjustment: (subLand / compLand)^elasticity
...
export function valueProperty(input) {
  ...
  // ── Large-Lot Detection (Phase 5): before the minimum-comps check ──
  // If large-lot mode is active, inject large-lot comparables into the main
  // list so they go through the normal valuation pipeline.
  ...
  // When large_lot_house mode is active, merge large-lot comparables into main list
  if (largeLotDetect.mode === "large_lot_house" && largeLotComps.length > 0) {
    ...
    // Add selected large-lot comps to comparables if not already present
    ...
    for (const llc of selectedLargeComps) {
      if (!existingAddrs.has(llc.address)) {
        // Tag with large_lot flag so we can skip them later for standard estimate
  ...
  if (largeLotDetect && largeLotDetect.mode === "large_lot_house" && input.largeLotComparables?.length > 0) {
    ...
    if (selectedLargeComps.length > 0) {
      ...
      if (combinedMidpoint) {
        ...
        const experimentalLabel = selectedLargeComps.length < 3
          ? `⚠️ 实验性结果 — 仅 ${selectedLargeComps.length} 个大块地可比`
          : null;
