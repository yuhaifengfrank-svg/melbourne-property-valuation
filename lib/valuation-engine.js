...
function anchorFactorAdjustments(anchor, subject, comparables, macro) {
  ...
  // ④ 市场动量（±5%）
  let momentum = 0;
  if (macro.cashRate != null) {
    // 当前均衡利率基准 3.5%（RBA 长期中性利率估计）
    // 高于基准 → 紧缩 → 下行压力
    // 低于基准 → 宽松 → 上行支持
    momentum += (3.5 - macro.cashRate) * 0.01;
  }
...
export function valueProperty(input) {
  ...
  if (macro.cashRate != null)
    effectiveGrowthRate = Math.max(effectiveGrowthRate + (3.5 - macro.cashRate) * 0.005, -0.02);
