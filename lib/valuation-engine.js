...
function confidenceResult(rows, rejectedRows, dispersion, sigma, vicplan = {}) {
  const coreRows = rows.filter((row) => row.qualityBand === "Core");
  ...
  // 住宅主导区域 → 可比数据更可靠，置信加权 +5
  const vicplanBoost = (vicplan.residentialDominant === true) ? 5 :
                        (vicplan.residentialDominant === false) ? -3 : 0;

  let label = "Low";
  if (coreRows.length >= 5 && recentRows.length >= 3 && crossCheckedRows.length >= 3 && dispersion <= 0.05) {
    label = "High";
  } else if (coreRows.length >= 3 && recentRows.length >= 2 && crossCheckedRows.length >= 2 && dispersion <= 0.075) {
    label = "Medium-High";
  } else if (rows.length >= 3 && averageScore >= 65 && dispersion <= 0.10) {
    label = "Medium";
  ...
  const dataScore = clamp(
    ...
    + coreRows.length * 7
    + recentRows.length * 4
    + crossCheckedRows.length * 5
    - rejectedRows.length * 1.5
    - dispersion * 100
    + vicplanBoost,
    0,
    100
  ...
  return {
    ...
    modelSigma: sigma,
    reasons: [
      `${rows.length} comparable sales accepted; ${rejectedRows.length} rejected.`,
