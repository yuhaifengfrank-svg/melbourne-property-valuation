...
const FACTOR_CAPS = {
  ...
  educationFactor: { min: -0.03, max: 0.05 },
...
function anchorFactorAdjustments(anchor, subject, comparables, macro) {
  ...
  // 每 ±50 分调 ±1%，最高 cap ±5%（最低 -3% 因为学校差只会降低到一定程度）
  const subIcsea = number(subject.avgIcsea);
  if (subIcsea) {
    const diff = subIcsea - 1000;
    factors.educationFactor = clamp(diff * 0.0002, FACTOR_CAPS.educationFactor.min, FACTOR_CAPS.educationFactor.max);
