...
const MAX_STANDARD_COMPARABLES = 24;
...
const TYPE_COMPATIBILITY = {
  ...
  Unit: new Set(["Unit", "Villa", "Townhouse"]),
...
function compareComparableRank(a, b) {
  if (b.weight !== a.weight) return b.weight - a.weight;
  const aDistance = a.distanceMeters == null ? Infinity : a.distanceMeters;
  const bDistance = b.distanceMeters == null ? Infinity : b.distanceMeters;
  if (aDistance !== bDistance) return aDistance - bDistance;
  if (a.ageMonths !== b.ageMonths) return a.ageMonths - b.ageMonths;
  return a.index - b.index;
}

function comparableTransactionKey(row) {
  const address = String(row.address || "")
    .normalize("NFKC")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim();
  // Transaction identity is based on address + date only.
  // salePrice is excluded because two sources may record conflicting
  // prices for the same transaction; keeping the higher-quality record
  // is handled by compareComparableRank, not by key collision.
  return `${address}|${row.saleDate || ""}`;
}
...
function scoreTypeMatch(subjectType, compType) {
  if (subjectType === compType) return 18;
  if (!typeMatches(subjectType, compType)) return 0;
  // 不同但兼容的类型：Unit→Townhouse 匹配度高于其他兼容组合
  if (subjectType === "Unit" && compType === "Townhouse") return 15;
  if (subjectType === "Townhouse" && compType === "Unit") return 15;
  return 10;
}
...
function scoreComparable(subject, comp, asOfDate) {
  ...
  const srcCount = Math.max(1, Number(comp.sourceCount) || (comp.crossChecked ? 2 : 1));
    ...
    const scores = {
    propertyType: scoreTypeMatch(subjectType, compType),
...
function confidenceResult(rows, rejectedRows, dispersion, macro) {
  ...
  const unknownDistanceRows = rows.filter(r => r.distanceMeters == null);
  const unknownDistanceRatio = rowCount ? unknownDistanceRows.length / rowCount : 1;
  ...
  // Candidates that passed validation but ranked below the final display cap
  // are screened-out alternatives, not rejected evidence. A larger candidate
  // pool must not reduce confidence merely because more valid rows were
  // considered before selecting the best 12.
  const rankedOutRows = rejectedRows.filter(row => row.reasons?.some(r => r.startsWith("candidate-rank-below")));
  const duplicateRows = rejectedRows.filter(row => row.reasons?.includes("duplicate-comparable-transaction"));
  const invalidRejectedRows = rejectedRows.filter(row =>
    !row.reasons?.some(r => r.startsWith("candidate-rank-below")) &&
    !row.reasons?.includes("duplicate-comparable-transaction")
  );
  const rankedOutCount = rankedOutRows.length;
  const rejP = Math.max(0, 10 - invalidRejectedRows.length * 2);

  const raw = (countS + coreB + recB + crossB + qualB + compB + vpBoost) * oPenalty * sPenalty;
  const distancePenalty = Math.round(unknownDistanceRatio * 15);
  const dataScore = clamp(Math.round(raw + rejP - distancePenalty), 0, 100);

  let label;
  if (dataScore >= 78) label = "High";
  else if (dataScore >= 65) label = "Medium-High";
  ...
  return { label, dataScore: round(dataScore),
    reasons: [
      `${rowCount} accepted; ${invalidRejectedRows.length} rejected.`,
      rankedOutCount ? `${rankedOutCount} valid candidates ranked below final selection.` : null,
      duplicateRows.length ? `${duplicateRows.length} duplicate transactions removed.` : null,
      `${coreRows.length} core >= 80.`,
      `${recentRows.length} ≤6mo.`,
      `${crossCheckedRows.length} cross-sourced.`,
      unknownDistanceRows.length
        ? `${unknownDistanceRows.length}/${rowCount} comparables have unknown distance; confidence reduced.`
        : null,
...
export function valueProperty(input) {
  ...
  // SEIFA: subject 上已有（从 census-service 注入）则优先，否则从 publicData 拿
  if (subject._seifaDecile == null && absProfile?.ok && absProfile.seifa?.decileAustralia != null)
  ...
  // Remove duplicate representations of the same transaction before ranking.
  // Keep the higher-quality record without using price as a ranking input.
  if (largeLotDetect.mode !== "large_lot_house" && accepted.length > 1) {
    const uniqueByTransaction = new Map();
    for (const candidate of accepted) {
      const key = comparableTransactionKey(candidate);
      const existing = uniqueByTransaction.get(key);
      if (!existing) {
        uniqueByTransaction.set(key, candidate);
        continue;
      }
      // Compare using quality rank (weight, distance, age, index).
      // Sale price must NOT be used to decide which record to keep.
      const keepCandidate = compareComparableRank(candidate, existing) < 0;
      const removed = keepCandidate ? existing : candidate;
      if (keepCandidate) uniqueByTransaction.set(key, candidate);
      rejected.push({
        index: removed.index,
        address: removed.address || "Unknown",
        sourceUrl: removed.sourceUrl || "",
        reasons: ["duplicate-comparable-transaction"],
        qualityScore: removed.qualityScore
      });
    }
    accepted.splice(0, accepted.length, ...uniqueByTransaction.values());
  }

  // Rank the broad database candidate pool only after every candidate has
  // passed validation and received the existing quality score. Price is
  // deliberately excluded from this ordering so selection cannot be tuned
  // toward a desired estimate. Stable fallbacks make repeated valuations
  // deterministic when quality scores tie.
  if (largeLotDetect.mode !== "large_lot_house" && accepted.length > MAX_STANDARD_COMPARABLES) {
    accepted.sort(compareComparableRank);

    const belowCut = accepted.splice(MAX_STANDARD_COMPARABLES);
    for (const comp of belowCut) {
      rejected.push({
        index: comp.index,
        address: comp.address || "Unknown",
        sourceUrl: comp.sourceUrl || "",
        reasons: ["candidate-rank-below-final-limit"],
  ...
  const unknownDistanceRatio = accepted.length
    ? accepted.filter(c => c.distanceMeters == null).length / accepted.length
    : 1;
  ...
  if (unknownDistanceRatio > 0) {
    sigma = Math.max(sigma, 0.08 + 0.06 * unknownDistanceRatio);
  }
  ...
  if (largeLotDetect && largeLotDetect.mode === "large_lot_house" && input.largeLotComparables?.length > 0) {
    ...
    if (selectedLargeComps.length > 0) {
      ...
      if (combinedMidpoint) {
        ...
        const subjectLat = Number(input.subject?.coordinates?.lat);
        const subjectLon = Number(input.subject?.coordinates?.lon);
        const coordinatesAvailable = Number.isFinite(subjectLat) && Number.isFinite(subjectLon)
          && subjectLat >= -39.3 && subjectLat <= -33.9
          && subjectLon >= 140.8 && subjectLon <= 150.1;
