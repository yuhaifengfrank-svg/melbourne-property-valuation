import { computeStreetQuality } from "./street-quality-service.js";
...
const FACTOR_CAPS = {
  ...
  streetQualityAdj: { min: -0.05, max: 0.05 },
  ...
  censusConsistency: { min: -0.05, max: 0.05 }
};
...
function scoreTypeMatch(subjectType, compType) {
  // Unit/Townhouse/Villa labels are not a proxy for size or accommodation.
  // Concrete area/room fields are scored separately below.
  if (subjectType === "Unit" && typeMatches(subjectType, compType)) return 15;
  if (subjectType === compType) return 18;
  if (!typeMatches(subjectType, compType)) return 0;
  // 不同但兼容的类型：Unit may use Townhouse/Villa as secondary evidence.
...
function scoreComparable(subject, comp, asOfDate) {
  ...
  const srcCount = Math.max(
    1,
    Number(comp.sourceCount) || ((comp.crossChecked || comp.verificationStatus === "cross_source_verified") ? 2 : 1)
  );
...
function anchorFactorAdjustments(anchor, subject, comparables, macro) {
  ...
  factors.streetQualityAdj = clamp((subStr - compStrMed) * 0.025, FACTOR_CAPS.streetQualityAdj.min, FACTOR_CAPS.streetQualityAdj.max);
  ...
  let momentum = 0;
  let md = [];
  // 房贷利率差：lending rate - cash rate = 银行利差
  // 利差扩大 → 信贷收紧 → 负面
  if (macro.mortgageRate != null && macro.cashRate != null) {
    const spread = macro.mortgageRate - macro.cashRate;
    if (spread > 3.5) {
      momentum += -0.01;
      md.push(`房贷利差${round(spread,1)}% 偏紧`);
    } else if (spread < 2.0) {
      momentum += 0.01;
      md.push(`房贷利差${round(spread,1)}% 偏宽`);
    } else {
      md.push(`房贷利差${round(spread,1)}%`);
    }
  }
  ...
  // SEIFA 社经指数（ABS 1-10，越高越富裕）
  const seifaDecile = subject._seifaDecile ?? null;
  if (seifaDecile != null) {
    if (seifaDecile >= 8) {
      censusAdj += 0.015;
      cdetails.push(`SEIFA ${seifaDecile}/10 高社经`);
    } else if (seifaDecile >= 5) {
      censusAdj += 0.005;
      cdetails.push(`SEIFA ${seifaDecile}/10 中等社经`);
    } else {
      censusAdj += -0.005;
      cdetails.push(`SEIFA ${seifaDecile}/10 社经偏弱`);
    }
  } else {
    cdetails.push("SEIFA 无数据");
  }
...
function confidenceResult(rows, rejectedRows, dispersion, macro) {
  ...
  const crossCheckedRows = rows.filter(r => r.sourceCount >= 2 || r.verificationStatus === "cross_source_verified");
  ...
  return { label, dataScore: round(dataScore),
    reasons: [`${rowCount} accepted; ${rejectedRows.length} rejected.`, `${coreRows.length} core >= 80.`, `${recentRows.length} ≤6mo.`, `${crossCheckedRows.length} cross-sourced.`, `Dispersion ${round(dispersion*100,1)}%.`].filter(Boolean) };
}
...
export function detectLargeLotMode(subject, landStats, isAddressLevelSource) {
  ...
  if (localMedian && (landSize / localMedian) >= 3.0) triggers.push("landSize / localMedian >= 3.0");
...
export function valueProperty(input) {
  ...
  const mortgageRate = rbaRates?.lendingRates?.stdVariableOwnerOccupier ?? rbaRates?.lendingRates?.discountedVariableOwnerOccupier ?? null;
  const macro = {
    cashRate: rbaRates?.cashRate ?? 4.31,  // fallback to current rate if RBA API unavailable
    mortgageRate,  // RBA F5: 标准可变自住贷款利率
  ...
  const effectiveGrowthRate = number(input.annualMarketGrowthRate) || 0;
  ...
  // ── Phase 1.5: 街道质量 ──
  // 使用已 scored 的 comparables 重新计算 subject 的街道质量
  try {
    if (subject.address) {
      const parts = (subject.address || "").split(",");
      const suburbName = subject.suburb || (parts.length > 1 ? parts[1].trim().split(/\s+/)[0] : "") || "";
      subject.streetQualityScore = computeStreetQuality(subject.address, suburbName, accepted);
    }
  } catch (_e) { /* 街道质量失败不影响主流程 */ }
