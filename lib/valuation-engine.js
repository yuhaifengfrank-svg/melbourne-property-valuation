...
export function channelAEstimate(subject, comparables, landElasticity = 0.196) {
  ...
  const adjusted = comparables.map(c => {
    ...
    // Time adjustment (approximate: -0.5%/month for >6 months)
    const ageM = number(c.ageMonths) || 0;
    const timeAdj = ageM <= 6 ? 1.0
      : ageM <= 12 ? 0.99
      : ageM <= 24 ? 0.97
      : ageM <= 36 ? 0.94
      : 0.90;
    ...
    const bedDiff = Math.abs(cBeds - subBed);
    const bathDiff = Math.abs(cBaths - subBath);
    const accomAdj = Math.max(0.90, 1.0 - (bedDiff * 0.02 + bathDiff * 0.015));

    // Condition adjustment
    const condDiff = Math.abs((number(c.conditionScore) ?? 3) - subCond);
    const condAdj = Math.max(0.90, 1.0 - condDiff * 0.02);

    // Total adjustment
    const totalAdj = distAdj * timeAdj * landAdj * accomAdj * condAdj;

    // Weight: land similarity dominant
    ...
    const landSimWeight = landRatioComp >= 0.80 && landRatioComp <= 1.25 ? 1.2
      ...
      : 0.6;
    const distW = dist === null ? 0.5 : Math.exp(-dist / 3000);
    const recW = Math.exp(-ageM / 730);
...
export function valueProperty(input) {
  ...
  // ── Large-Lot Detection (Phase 5) ──
  // Runs in parallel with standard method; does not replace it
  ...
  if (largeLotDetect.mode === "large_lot_house" && largeLotComps.length > 0) {
    // Select best large-lot comparables
