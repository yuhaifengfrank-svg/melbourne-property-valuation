 ...
 *
 * IMPORTANT: alreadyPurchased is ONLY true when there's an active entitlement.
 * A paid payment without an active entitlement → paymentPresent: true, alreadyPurchased: false.
 * This prevents double-charging while allowing the caller to detect the state.
 */
export async function ensureReportPayment(opts, sql) {
  ...
  if (entitlements.length > 0) {
    return { payment: null, alreadyPurchased: true, alreadyConsumed: false, paymentPresent: false };
  }
  ...
  if (existingPayments.length > 0) {
    ...
    // Already paid but no active entitlement — don't charge again but don't claim purchased
    // Caller should return PAYMENT_AWAITING_ENTITLEMENT
    if (existing.status === "paid") {
      return { payment: existing, alreadyPurchased: false, alreadyConsumed: true, paymentPresent: true };
    }

    // Pending with a session ID — return as-is (idempotency)
    if (existing.status === "pending" && existing.stripe_checkout_session_id) {
      return { payment: existing, alreadyPurchased: false, alreadyConsumed: true, paymentPresent: false };
    }
  ...
  return { payment: inserted[0], alreadyPurchased: false, alreadyConsumed: false, paymentPresent: false };
}
