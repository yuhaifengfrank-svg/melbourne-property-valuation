...
export async function handleChargeRefunded(charge, sql) {
  ...
  try {
    ...
    //
    // Re-check the current state to determine what happened and
    // provide a meaningful error to the caller.
    ...
    if (recheck.length > 0 && recheck[0].status === "refunded") {
      // Another concurrent caller already refunded it
      // Entitlement was also revoked atomically by them
      return {
        refunded: true,
        revoked: true,
        alreadyRefunded: true,
