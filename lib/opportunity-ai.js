...
export function generateExplanation(suburb, metrics) {
  ...
  const vacancy = metrics.vacancy_rate;
  ...
  if (g3 != null && g3 > 0) {
    rankReasons.push(`With ${g3}% growth over 3 years, the suburb demonstrates strong price appreciation trends.`);
  } else if (g3 != null) {
    rankReasons.push(`The 3-year growth of ${g3}% suggests a cooling market — value-focused investors may find entry opportunities.`);
  }
  ...
  if (vacancy != null && vacancy < 3) rankReasons.push(`Low vacancy rate of ${vacancy}% indicates strong rental demand.`);
  ...
  // ── Risks ──
  const riskList = [];
  if (g3 != null && g3 < -5) riskList.push('Price decline of ' + g3 + '% over 3 years — capital loss risk if cycle continues.');
  if (g3 != null && g3 > 20) riskList.push('Rapid price growth of ' + g3 + '% may not be sustainable — correction risk exists.');
  if (vacancy != null && vacancy > 5) riskList.push('Above-average vacancy rate of ' + vacancy + '% could indicate weak rental demand.');
