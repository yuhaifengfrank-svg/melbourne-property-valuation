 ...
 *   3. growth-projector   → growth_1y, growth_3y, growth_5y
 *   4. salm_sa2_data (SALM) → supply_unemployment_rate, supply_employment_growth, conf_income
 *   5. opportunity-scoring-v2 → opportunity_score, opportunity_type
 *
 * Rental metrics are intentionally excluded. Census weekly rent is not a
 * current 3/4-bedroom house rent and Census-night unoccupied dwellings are not
 * rental vacancies. Source-aware rental observations use the separate suburb
 * market-data pipeline and must pass publication gates before use.
 *
 * Safety:
 *   - Does NOT TRUNCATE. Uses INSERT ON CONFLICT (suburb, state) DO UPDATE.
 *   - Existing rental/cagr/growth fields are not overwritten with NULLs.
...
async function refreshSuburbMetrics() {
  ...
  // Step 1: Upsert base metrics from comparable_sales and schools.
  ...
  // Step 2: Run growth projection for all suburbs with data
  ...
  // Step 3: Populate SALM employment/unemployment data from salm_sa2_data
  //   Replaces Census G46 2021 data with much fresher quarterly estimates.
  //   Updates explicit employment count/growth fields. conf_income is Census
  //   income confidence and must not be inferred from unemployment.
  ...
  await q(`
    ...
    SET
      ...
      supply_employment_count = COALESCE(salm.employment_count, sm.supply_employment_count)::numeric,
      supply_employment_growth_yoy = COALESCE(salm.employment_growth_yoy, sm.supply_employment_growth_yoy)::numeric,
      supply_employment_growth = COALESCE(salm.employment_growth_yoy, sm.supply_employment_growth)::numeric,
      supply_employment_period = COALESCE(salm.latest_quarter, sm.supply_employment_period),
    ...
    WHERE LOWER(sm.suburb) = salm.lsuburb
      AND sm.state = 'VIC'
      AND NOT EXISTS (
        SELECT 1 FROM suburb_sa2_membership membership
        WHERE LOWER(membership.suburb)=LOWER(sm.suburb) AND membership.state=sm.state
      )
  `);
  await q(`
    WITH member_values AS (
      SELECT membership.suburb,membership.state,salm.latest_quarter,
        COUNT(*)::int member_count,
        SUM(salm.labour_force)::numeric labour_force,
        SUM(salm.unemployed)::numeric unemployed,
        MAX(salm.unemployment_rate)::numeric reported_unemployment_rate,
        SUM(salm.employment_count)::numeric employment_count,
        SUM((salm.historical_data->salm.employment_growth_base_quarter->>'lf')::numeric
          - (salm.historical_data->salm.employment_growth_base_quarter->>'unemp')::numeric)::numeric base_employment
      FROM suburb_sa2_membership membership JOIN salm_sa2_data salm USING(sa2_code)
      GROUP BY membership.suburb,membership.state,salm.latest_quarter
    )
    UPDATE suburb_metrics target SET
      supply_unemployment_rate=ROUND(CASE WHEN source.member_count=1 THEN source.reported_unemployment_rate ELSE source.unemployed/NULLIF(source.labour_force,0)*100 END,2),
      supply_employment_count=source.employment_count,
      supply_employment_growth_yoy=(source.employment_count/NULLIF(source.base_employment,0)-1)*100,
      supply_employment_growth=(source.employment_count/NULLIF(source.base_employment,0)-1)*100,
      supply_employment_period=source.latest_quarter,
      supply_unemployment_quarter=source.latest_quarter,
      conf_income=80,
      updated_at=NOW()
    FROM member_values source
