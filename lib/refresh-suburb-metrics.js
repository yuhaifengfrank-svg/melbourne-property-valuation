...
async function refreshSuburbMetrics() {
  ...
  const insertSql = `
    ...
    WITH
      ...
      -- Vacancy rate: G36 Unoccupied_PDs_Dwgs / Total_PDs_Dwellings via SA2 mapping
      vac AS (
        SELECT DISTINCT ON (sl.suburb)
               sl.suburb,
               ROUND(
                 ((census.g36->>'Unoccupied_PDs_Dwgs')::numeric 
                   / NULLIF((census.g36->>'Total_PDs_Dwellings')::numeric, 0)) * 100,
                 2
               ) AS vac_rate
        FROM school_locations sl
        JOIN census_sa2_data census ON sl.sa2_code = census.sa2_code::text
    ...
    SELECT
      ...
      END,
      -- vacancy_rate from G36 census data
      vac.vac_rate,
