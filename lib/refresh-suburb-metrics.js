...
async function refreshSuburbMetrics() {
  ...
  const upsertSql = `
    ...
    WITH
      ...
      map_sa2 AS (
        SELECT LOWER(m.suburb) AS suburb, m.sa2_code
        FROM suburb_sa2_map m
        UNION
        SELECT LOWER(sl.suburb) AS suburb, sl.sa2_code
        FROM school_locations sl
        WHERE NOT EXISTS (SELECT 1 FROM suburb_sa2_map m WHERE LOWER(m.suburb) = LOWER(sl.suburb))
      ),
      rent AS (
        SELECT map.suburb,
               ROUND(AVG((census.g02->>'Median_rent_weekly')::numeric)) AS median_rent_weekly
        FROM map_sa2 map
        JOIN census_sa2_data census ON map.sa2_code = census.sa2_code::text
  ...
  // Step 5.10: Refresh planning metadata (zone_code, overlays, heritage) from planning_cache + school_locations
  console.log('[refresh] Refreshing planning metadata...');
  try {
    const allPc = await q('SELECT lat_lon_key, latitude, longitude, zone_code, overlays FROM planning_cache', []);
    const pcByKey = new Map(allPc.map(r => [r.lat_lon_key, r]));

    function gridRound(v, step) { return (Math.round(Number(v) / step) * step).toFixed(4); }
    ...
    const suburbs = await q('SELECT sm.suburb FROM suburb_metrics sm WHERE sm.state = $1', ['VIC']);
    let planned = 0;
    for (const s of suburbs) {
      const schoolRows = await q(`
        SELECT ROUND(AVG(latitude)::numeric, 4) AS lat, ROUND(AVG(longitude)::numeric, 4) AS lon
        FROM school_locations
        WHERE LOWER(suburb) = $1 AND state = 'VIC' AND latitude IS NOT NULL
        GROUP BY LOWER(suburb)
      `, [s.suburb]);

      let zoneCode = null, overlays = null, vhrFlagged = false, heritageLevel = 'none';
      ...
      if (schoolRows.length > 0) {
        ...
        // Try exact 4dp key first
        const rawKey = lat.toFixed(4) + '_' + lon.toFixed(4);
        bestRow = pcByKey.get(rawKey) || null;
        if (bestRow) bestDist = haversineKm(lat, lon, parseFloat(bestRow.latitude), parseFloat(bestRow.longitude));

        // Fallback: SQL nearest-neighbor on planning_cache
        if (!bestRow || bestDist > 0.15) {
          try {
            const nearest = await q(`
              SELECT lat_lon_key, zone_code, overlays, latitude, longitude
              FROM planning_cache
              ORDER BY ABS(latitude::numeric - $1) * ABS(latitude::numeric - $1) + ABS(longitude::numeric - $2) * ABS(longitude::numeric - $2)
              LIMIT 3
            `, [lat, lon]);
            for (const candidate of nearest) {
              const d = haversineKm(lat, lon, parseFloat(candidate.latitude), parseFloat(candidate.longitude));
              if (d < bestDist) { bestDist = d; bestRow = candidate; }
            }
          } catch (e) {
            // SQL fallback failed; try broader in-memory
        ...
        if (bestRow && bestDist < 2.0) {
          zoneCode = bestRow.zone_code;
          overlays = bestRow.overlays;

          try {
            const vhrCheck = await q(`SELECT COUNT(*)::int AS cnt FROM vhr_zones WHERE ST_Contains(geom, ST_SetSRID(ST_MakePoint($1, $2), 4326))`, [lon, lat]);
            vhrFlagged = vhrCheck[0].cnt > 0;
          } catch (e) { /* PostGIS not available */ }

          let hasHO = false;
          if (overlays) {
            const codes = Array.isArray(overlays) ? overlays.map(o => (o && o.code) || '') : [];
            hasHO = codes.some(c => c === 'HO' || c.startsWith('HO'));
          }
          heritageLevel = vhrFlagged ? (hasHO ? 'both' : 'state') : hasHO ? 'local' : 'none';
        }
      }

      await q(`
        UPDATE suburb_metrics SET
          planning_zone_code = $1,
          planning_overlay_codes = $2::jsonb,
          planning_heritage_level = $3,
          vhr_flagged = $4,
          updated_at = NOW()
        WHERE suburb = $5 AND state = 'VIC'
      `, [zoneCode, overlays ? JSON.stringify(overlays) : null, heritageLevel, vhrFlagged, s.suburb]);
      planned++;
    }
    console.log('[refresh] Updated planning metadata for ' + planned + '/' + suburbs.length + ' suburbs');
  } catch (err) {
    console.warn('[refresh] Planning metadata refresh failed (non-fatal):', err.message);
  }

  // Step 6: Compute development + confidence scores (reads suburb_metrics, writes scores)
  console.log('[refresh] Computing development & confidence scores...');
  try {
    const { computeDevelopmentScoreFromMetrics } = await import('./development-intelligence.js');
    const suburbs = await q('SELECT * FROM suburb_metrics WHERE state = $1', ['VIC']);
    let devUpdated = 0;
    const updateSql = `
      UPDATE suburb_metrics SET
        development_score = $1,
    ...
    `;
    for (const s of suburbs) {
      const r = computeDevelopmentScoreFromMetrics(s);
      if (!r || r.developmentScore == null) continue;
      await q(updateSql, [
        r.developmentScore,
        r.developmentLabel || null,
        r.confidenceScore,
        r.confidenceLabel || null,
        s.suburb
      ]);
      devUpdated++;
    }
    console.log('[refresh] Updated ' + devUpdated + '/' + suburbs.length + ' suburb development scores');
  } catch (err) {
    console.warn('[refresh] Development scoring failed (non-fatal):', err.message);
  }

  // Step 7: Run opportunity scoring v2 (reads suburb_metrics, writes scores)
