...
export const FUTURE_OUTLOOK_MODEL_VERSION = "future_outlook_v1";
...
export function scoreFutureOpportunity(suburb, options = {}) {
  ...
  const componentScores = {
    affordability: scoreAffordability(metrics),
  ...
  const weighted = weightedAverage(componentScores, weights);
...
export function scorePropertyFutureOpportunity({ suburbOutlook, property = {} }) {
  ...
  const futureOpportunityIndex = Math.round(clamp(base * 0.70 + propertySpecificScore * 0.30, 0, 100));
...
function normaliseSuburbMetrics(suburb = {}, propertyType) {
  ...
  return {
    ...
    // 新: 收入/租金/利率——从 suburb 对象传进来
    hhdIncome: num(suburb.hhdIncome ?? suburb.hhd_income),  // 年家庭收入中位数
    medianRent: num(suburb.median_rent_dffh ?? suburb.medianRent ?? suburb.median_rent ?? suburb.medianHouseRent ?? suburb.median_house_rent), // 周租金（DFFH 优先）
    mortgageRate: num(suburb.mortgageRate ?? suburb.mortgage_rate), // 抵押贷款利率(小数)
    vicMedianHousePrice: num(suburb.vicMedianHousePrice ?? suburb.vic_median_house_price),
  };
}

function scoreAffordability(metrics) {
  if (!metrics || metrics.price == null || metrics.price <= 0) return null;

  // ---- 默认值 ----
  const price = metrics.price;
  const income = metrics.hhdIncome;          // 年家庭收入中位数
  const rent = metrics.medianRent;           // 周租金
  const vicMedianPrice = metrics.vicMedianHousePrice;

  if (!income || income <= 0) {
    // 无收入数据时 fallback 到旧 price-only 模型
    return scoreLine(price, {
      weak: 1800000, market: 1200000,
      excellent: 750000, exceptional: 500000,
      lowerIsBetter: true,
    });
  }
  ...
  // ① 房价收入比 (PIR) — 35%
  // 全球基准：≤3x=100, 5x=80, 10x=50, 15x=20, ≥20x=0
  const pir = price / income;
  const pirScore = scoreLine(pir, {
    weak: 15, market: 10, excellent: 5, exceptional: 3, lowerIsBetter: true,
  });

  // ② 按揭压力 — 30%
  // 月供 = price * 利率 / 12 * (利率*(1+利率)^n) / ((1+利率)^n-1)
  // 简化为: 年供 ≈ price * (rate + 0.02)  (考虑本金+利息, 30年贷款)
  const rate = metrics.mortgageRate || 0.0630;  // 当前抵押贷款利率 ~6.3%
  const annualRepayment = price * rate;          // 简化: 只还利息
  const dti = annualRepayment / income;          // debt-to-income ratio
  // DTI: ≤0.2=100, 0.3=80, 0.4=50, 0.5=20, ≥0.6=0
  const mortgageScore = scoreLine(dti, {
    weak: 0.5, market: 0.4, excellent: 0.3, exceptional: 0.2, lowerIsBetter: true,
  });

  // ③ 首付压力 — 15%
  // 首付 = price * 20%, 首付年数 = 首付 / (income * 储蓄率)
  const depositRate = 0.20;
  const savingsRate = 0.15;  // 假设存15%收入
  const depositYears = (price * depositRate) / (income * savingsRate);
  // ≤2年=100, 4年=80, 7年=50, 10年=20, ≥15年=0
  const depositScore = scoreLine(depositYears, {
    weak: 10, market: 7, excellent: 4, exceptional: 2, lowerIsBetter: true,
  });

  // ④ 租金压力 — 10%
  // 租金收入比 = rent * 52 / income
  if (rent && rent > 0) {
    var rti = (rent * 52) / income;
  } else {
    var rti = null;
  }
  const rentScore = rti != null
    ? scoreLine(rti, {
        weak: 0.35, market: 0.30, excellent: 0.20, exceptional: 0.15, lowerIsBetter: true,
      })
    : null;

  // ⑤ 相对州中位数 — 10%
  // price / vicMedianPrice
  if (vicMedianPrice && vicMedianPrice > 0) {
    var relativePrice = price / vicMedianPrice;
  } else {
    var relativePrice = 1.0;
  }
  const relativeScore = scoreLine(relativePrice, {
    weak: 1.8, market: 1.3, excellent: 0.9, exceptional: 0.6, lowerIsBetter: true,
  });
  ...
  let numerator = 0;
  let denom = 0;
  for (const [key, value] of Object.entries({ pir: pirScore, mortgage: mortgageScore, deposit: depositScore, rent: rentScore, relative: relativeScore })) {
    if (value != null) {
  ...
  const combined = denom > 0 ? Math.round(clamp(numerator / denom, 0, 100)) : null;
  return combined;
}
