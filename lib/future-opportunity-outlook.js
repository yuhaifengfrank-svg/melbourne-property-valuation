...
import { percentileScores, relativeRating, roundRelativeScore } from "./relative-score.js";
...
export function scoreFutureOpportunity(suburb, options = {}) {
  ...
  score = roundRelativeScore(clamp(score, 0, 100));
  confidenceScore = roundRelativeScore(clamp(confidenceScore, 0, 100));
  ...
  return {
    ...
    componentRatings: Object.fromEntries(
      Object.entries(componentScores).map(([key, value]) => [key, relativeRating(value)]),
    ),
...
export function calibrateFutureOpportunityOutlooks(outlooks = []) {
  const scores = percentileScores(outlooks.map((outlook) => outlook?.futureOpportunityIndex));
  ...
  return outlooks.map((outlook, index) => {
    ...
    return {
      ...
      rating: relativeRating(futureOpportunityIndex),
